...
class GraphBoardTest(GraphicUnitTest):
    ...
    def test_select_arrow():
        ...
        boardview = GraphBoardView(board=board)
        win = window_with_widget(boardview)
        idle_until(lambda: board.arrow_plane, 100, "GraphBoard never got arrow_plane")
        idle_until(
    ...
    def test_spot_and_pawn_from_dummy():
        ...
        idle_until(lambda: view.plane is not None, 100, "Never made BoardScatterPlane")
        idle_until(lambda: board.stack_plane is not None, 100, "Never made StackPlane")
        ...
        idle_until(lambda: dummy in board.children, 100, "Dummy didn't get to board")
        ...
        idle_until(lambda: dummy2_name in char.thing, 100, "Dummy 2 didn't add thing")
        idle_until(
...
class SwitchGraphTest(ELiDEAppTest):
    def test_character_switch_graph(self):
        ...
        assert len(
            set(
                child.x for child in app.mainscreen.boardview.board.stack_plane.children
            )
