...
from LiSE.examples import kobold, polygons
...
class RuleBuilderTest(ELiDEAppTest):
    ...
    def setUp(self):
        ...
        idle_until(lambda: "ruleslist" in rules_box.ids, 100, "Never made rules list")
...
class TestRuleBuilderKobold(RuleBuilderTest):
    ...
    def test_rule_builder_display_trigger(self):
        ...
        idle_until(
            lambda: [child for child in builder.children if isinstance(child, Card)],
            100,
            "Never filled trigger builder",
        )
        card_names = {
            card.headline_text for card in builder.children if isinstance(card, Card)
        }
        assert card_names == {
            "standing_still",
            "aware",
    ...
    def test_rule_builder_remove_trigger(self):
        ...
        idle_until(breakcover_is_flush_with_right_foundation, 100, "card didn't move")
        idle_until(
            lambda: "breakcover" not in self.app.engine.rule["shrubsprint"].triggers,
            100,
...
class TestCharRuleBuilder(ELiDEAppTest):
    def setUp(self):
        ...
        with Engine(self.prefix) as eng:
            ...
            assert list(eng.character["triangle"].unit.rule["relocate"].triggers) == [
