    ...
    * ``footer_text``, like ``headline_text`` but at the bottom
    ...
    def on_text(self, *_):
        ...
        if self.markup:
            ...
            text = text.replace("[", "\x01").replace("]", "\x02").replace("\t", " " * 4)
...
class DeckBuilderLayout(Layout):
    ...
    direction = OptionProperty("ascending", options=["ascending", "descending"])
    ...
    def on_touch_move(self, touch):
        ...
        if (
            ...
            return
        if touch.ud["layout"] == self and not hasattr(touch.ud["card"], "_topdecked"):
        ...
        for i, deck in enumerate(self.decks):
            ...
            if self.direction == "descending":
                cards.reverse()
            cards_collided = [card for card in cards if card.collide_point(*touch.pos)]
            if cards_collided:
                ...
                if collided.deck == touch.ud["deck"]:
                    ...
                    self.insertion_card = (
                        1
                        if collided.idx == 0
                        else maxidx + 1 if collided.idx == maxidx else collided.idx + 1
                    )
...
class DeckBuilderScrollBar(FloatLayout):
    ...
    vbar = AliasProperty(_get_vbar, None, bind=("_scroll", "scroll_min", "scroll_max"))
    ...
    hbar = AliasProperty(_get_hbar, None, bind=("_scroll", "scroll_min", "scroll_max"))
