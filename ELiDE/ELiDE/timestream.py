...
class ThornyRectangle(Button):
    ...
    def _redraw(self, *_):
        ...
        self._redraw_line(self.draw_left, "_left_line", self._get_left_line_points)
        self._redraw_line(self.draw_right, "_right_line", self._get_right_line_points)
        self._redraw_line(self.draw_up, "_top_line", self._get_top_line_points)
        self._redraw_line(self.draw_down, "_bot_line", self._get_bottom_line_points)
    ...
    for row, (branch, _) in enumerate(sorted_branches):
        for turn in col2turn:
            if branch == "trunk" and turn == 0:
                ...
                here_branches = [(branches[b][1], b) for b in start_turn_branches[turn]]
