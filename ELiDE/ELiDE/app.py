...
from LiSE.proxy import CharStatProxy, EngineProcessManager, PlaceProxy, ThingProxy
...
class ELiDEApp(App):
    ...
    def on_selected_proxy(self, *_):
        ...
        assert hasattr(selected_proxy, "destination"), "{} has no destination".format(
            type(selected_proxy)
        )
        origin = selected_proxy.origin
        destination = selected_proxy.destination
        self.selected_proxy_name = str(origin.name) + "->" + str(destination.name)
    ...
    def _really_time_travel(self, branch, turn, tick):
        try:
            ...
            Logger.warning(
                f"App: couldn't time travel to {(branch, turn, tick)}: " + ex.args[0],
                exc_info=ex,
            )
    ...
    def init_board(self, *_):
        ...
        if "boardchar" not in self.engine.eternal:
            if "physical" in self.engine.character:
                ...
                chara = self.engine.eternal["boardchar"] = self.engine.new_character(
                    "physical"
                )
        self.chars.names = list(self.engine.character)
    ...
    def _add_screens(self, *_):
        ...
        self.funcs = ELiDE.stores.FuncsEdScreen(name="funcs", toggle=toggler("funcs"))
    ...
    def on_edit_locked(self, *_):
        Logger.debug(
            "ELiDEApp: " + ("edit locked" if self.edit_locked else "edit unlocked")
        )
