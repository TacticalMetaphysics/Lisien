...
class GraphArrow:
    ...
    def reciprocal(self) -> Optional["GraphArrow"]:
        ...
        if self.destination.name not in self.board.pred_arrow:
            return
        if self.origin.name not in self.board.pred_arrow[self.destination.name]:
            return
    ...
    def repoint(self, selected: bool = None) -> None:
        ...
        if (
            ...
            plane._instructions_map[self.origin.name, self.destination.name] = insts = (
                get_instructions(
                    *shaft_points,
                    *head_points,
                    bg_color,
            ...
            plane._colliders_map[self.origin.name, self.destination.name].points = (
                Collide2DPoly(points=insts["shaft_bg"].points)
            )
...
class ArrowPlane(Widget):
    ...
    def redraw(self, *args):
        ...
        points_map = get_points_multi(
            (datum["origspot"], datum["destspot"], taillen) for datum in self.data
        )
        port_l = []
        bot_left_corner_xs = []
        bot_left_corner_ys = []
        ...
        for (
            ...
            datum,
        ) in zip(lengths, points_map.items(), self.data):
            if length < r:
                continue
            ...
            colliders_map[port] = Collide2DPoly(points=instructions["shaft_bg"].points)
        fbo.release()
    ...
    def add_new_portal(self, datum: dict) -> None:
        ...
        instructions = self._instructions_map[orig_spot.name, dest_spot.name] = (
            get_instructions(
                *shaft_points,
                *head_points,
                bgr,
    ...
    def iter_collided_edges(self, x: float, y: float) -> Iterator:
        ...
        for port in map(itemgetter(0), filter(itemgetter(1), zip(self._port_l, hits))):
