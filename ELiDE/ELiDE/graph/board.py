...
class GraphBoard(RelativeLayout):
    ...
    def on_touch_down(self, touch):
        ...
        if spots:
            ...
            if self.adding_portal:
                ...
                self.protodest = Dummy(name="protodest", pos=touch.pos, size=(0, 0))
        ...
        try:
            ...
            if orig != dest and not (
                ...
                if symmetrical:
                    ...
                    self.arrow[dest.name][orig.name] = self.pred_arrow[orig.name][
                        dest.name
                    ] = GraphArrow(
                        board=self,
    ...
    def on_touch_up(self, touch):
        ...
        def unsel_graph_arrow():
            ...
            insts = self.arrow_plane._instructions_map[origspot.name, destspot.name]
        ...
        if self.app.selection:
            ...
            if isinstance(sel, Widget):
                ...
                if hasattr(sel.proxy, "location"):
                    for candidate in self.stack_plane.iter_collided_keys(*touch.pos):
    ...
    def make_arrow(self, portal):
        if portal["origin"] not in self.spot or portal["destination"] not in self.spot:
            raise ValueError(
    ...
    def remove_absent_pawns(self, *args):
        Logger.debug("Board: removing pawns absent from {}".format(self.character.name))
    ...
    def remove_absent_spots(self, *args):
        Logger.debug("Board: removing spots absent from {}".format(self.character.name))
        ...
        for arrow_origin in list(self.arrow.keys()):
            for arrow_destination in list(self.arrow[arrow_origin].keys()):
                if (
                    ...
                    or arrow_destination not in self.character.portal[arrow_origin]
                ):
    ...
    def add_new_spots(self, *args):
        Logger.debug("Board: adding new spots to {}".format(self.character.name))
        start_ts = monotonic()
        places2add = []
        spots_unposd = []
        nodes_patch = {}
    ...
    def add_arrow(self, orign, destn, *args):
        if not (
            orign in self.character.portal and destn in self.character.portal[orign]
        ):
    ...
    def add_new_arrows(self, *args):
        Logger.debug("Board: adding new arrows to {}".format(self.character.name))
        ...
        for arrow_orig, arrow_dests in portmap.items():
            for arrow_dest, portal in arrow_dests.items():
                if arrow_orig not in arrowmap or arrow_dest not in arrowmap[arrow_orig]:
    ...
    def add_new_pawns(self, *args):
        Logger.debug("Board: adding new pawns to {}".format(self.character.name))
        nodes_patch = {}
        things2add = []
        pawns_added = []
        pawnmap = self.pawn
    ...
    def update(self, *args):
        ...
        Logger.debug(f"GraphBoard: updated, took {monotonic() - start_ts:,.2f} seconds")
    ...
    def update_from_character_edge(self, edge, key, value):
        if edge:
            if not self.arrow_plane.have_arrow(edge._origin, edge._destination):
    ...
    def _apply_node_layout(self, l, spot, *args):
        if self.width == 1 or self.height == 1:
            Clock.schedule_once(partial(self._apply_node_layout, l, spot), 0.01)
            return
    ...
    def nx_layout(self, spots, *args):
        ...
        for place in spots_only.place.keys() - set(spot["name"] for spot in spots):
    ...
    def pawns_at(self, x, y):
        ...
        for name in self.pawn.keys() & set(self.stack_plane.iter_collided_keys(x, y)):
            yield self.pawn[name]

    def spots_at(self, x, y):
        """Iterate over spots that collide the given point."""
        for name in self.spot.keys() & set(self.stack_plane.iter_collided_keys(x, y)):
