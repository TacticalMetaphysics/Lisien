...
class TextureStackPlane(Widget):
    ...
    def redraw(self, *_):
        def get_rects(datum):
            ...
            for texture in datum["textures"]:
                ...
                if "height" in datum:
                    ...
                    height = h
                assert w > 0 and h > 0
                rects.append(Rectangle(pos=(x, y), size=(w, h), texture=texture))
            return rects
        ...
        Logger.debug(
            f"TextureStackPlane: redrawn in " f"{monotonic() - start_ts:,.2f} seconds"
        )
...
class GraphPawnSpot(ImageStackProxy, Layout):
    ...
    def finalize(self, initial=True):
        ...
        self._push_offxs_binding = self.fbind("offxs", self._trigger_push_offxs)
        self._push_offys_binding = self.fbind("offys", self._trigger_push_offys)
...
class Stack:
    ...
    def pos(self):
        ...
        return float(stack_plane._left_xs[idx]), float(stack_plane._bot_ys[idx])
