...
class TimePanel(BoxLayout):
    ...
    def set_tick(self, *_):
        ...
        self.screen.app.time_travel(self.screen.app.branch, self.screen.app.turn, tick)
        self.screen.charmenu.switch_to_menu()
    ...
    def on_screen(self, *_):
        if not all(
            field in self.ids for field in ("branchfield", "turnfield", "tickfield")
        ):
...
class MainScreen(Screen):
    ...
    def on_mainview(self, *_):
        ...
        self.calendar_view = ScrollView(size=self.mainview.size, pos=self.mainview.pos)
    ...
    def update_from_time_travel(self, command, branch, turn, tick, result, **kwargs):
...
class CharMenuContainer(BoxLayout):
    ...
    def _toggle_stepper(self, *_):
        if self.charmenu in self.children:
            ...
            self.clear_widgets()
            self.stepper.from_rules_handled_turn(engine.handle("rules_handled_turn"))
...
class TurnScroll(Slider):
    ...
    def on_touch_move(self, touch):
        if touch.grab_current == self:
            app = App.get_running_app()
            app.mainscreen.timepanel.ids.turnfield.hint_text = str(int(self.value))
