...
class Editor(BoxLayout):
    ...
    def save(self, *_):
        ...
        if self.name_wid is None or self.store is None:
            Logger.debug(
                "{}: Not saving, missing name_wid or store".format(type(self).__name__)
            )
            return
        ...
        if hasattr(self, "_do_parse"):
            try:
                ...
                Logger.debug(
                    "{}: Not saving, couldn't parse".format(type(self).__name__)
                )
                return
        do_redata = False
...
class EdBox(BoxLayout):
    ...
    def _pull_from_storelist(self, *_):
        ...
        try:
            self.editor.source = getattr(self.store, self.editor.name_wid.hint_text)
...
def munge_source(v):
    ...
    if not lines:
        return tuple(), ""
    params = tuple(parm.strip() for parm in sig_ex.match(lines[0]).group(1).split(","))
    del lines[0]
    if not lines:
        return params, ""
...
class FuncsEdBox(EdBox):
    ...
    def valid_name(name):
        return (
            name
            and name[0] not in string.digits + string.whitespace + string.punctuation
        )
