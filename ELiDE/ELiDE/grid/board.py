...
class GridBoard(Widget):
    ...
    def update(self):
        ...
        self.spot_plane.data = list(map(make_spot, self.character.place.values()))
        self.pawn_plane.data = list(map(make_pawn, self.character.thing.values()))
    ...
    def on_parent(self, *args):
        ...
        Logger.debug(
            f"GridBoard: on_parent end, took {monotonic() - start_ts:,.2f}" f" seconds"
        )
...
class GridBoardScatterPlane(BoardScatterPlane):
    ...
    def pawn_from_dummy(self, dummy):
        ...
        candidates = list(self.board.spot_plane.iter_collided_keys(*dummy_center))
        if not candidates:
            return
