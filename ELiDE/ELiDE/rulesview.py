...
class RulesView(Widget):
    ...
    def finalize(self, *_):
        ...
        self._tabs = TabbedPanel(size=self.size, pos=self.pos, do_default_tab=False)
    ...
    def _edit_something(self, card: Card):
        ...
        if not hasattr(self, "rule_func_editor_modal"):
            ...
            self.rule_func_editor = FuncEditor(size_hint_y=0.9, deletable=False)
            ...
            self.rule_func_editor_modal.add_widget(self.rule_func_editor_layout)
    ...
    def pull_prereqs(self, *args):
        ...
        self._prereq_builder.decks = self._pull_functions("prereq", truth=False)

    _trigger_pull_prereqs = trigger(pull_prereqs)

    def pull_actions(self, *args):
        """Refresh the cards in the action builder"""
        self._action_builder.decks = self._pull_functions("action", truth=False)
