...
:class:`.window.AssignmentTimeDict` holds a variable's value for
each turn in a pair of stacks, which in turn hold the basic
:class:`.window.WindowDict`, a pair of stacks kept in
...
#. Find the relevant :class:`.window.AssignmentTimeDict` for the desired branch and variable
   (generally a couple of plain dictionary lookups)

#. Pop/append that :class:`.window.AssignmentTimeDict` until the "past" stack's top entry is
...
#. Take the :class:`.window.WindowDict` from the top of the :class:`.window.AssignmentTimeDict`'s "past"
...
one big global :class:`.window.AssignmentTimeDict` when they are set or loaded. Then, to make
the delta, we take a slice of that global :class:`.window.AssignmentTimeDict` and compile the
