...
class HostPython3Recipe(Recipe):
	...
	url = "https://www.python.org/ftp/python/{version}/Python-{version}{version_suffix}.tgz"
	"""The default url to download our host python recipe. This url will
    change depending on the python version set in attribute :attr:`version`."""

	patches = ["patches/pyconfig_detection.patch", "patches/arm_machine.patch"]
	...
	def versioned_url(self):
		...
		return self.url.format(
			version=self.version, version_suffix=self.version_suffix
		)
	...
	def build_arch(self, arch):
		...
		with current_directory(build_dir):
			if not Path("config.status").exists():
				shprint(
					sh.Command(join(recipe_build_dir, "configure")), _env=env
				)
