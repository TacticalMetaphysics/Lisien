...
from .util import ELiDEAppTest, idle_until, advance_frames
...
@pytest.fixture
def kobold_state(play_dir):
	with Engine(play_dir, workers=0) as eng:
		kobold.inittest(eng)


@pytest.fixture
def rule_builder_app(kobold_state, elide_app):
	app = elide_app
	...
	idle_until(
		lambda: app.manager.current == "rules",
		100,
		"Never switched to rules screen",
	)
	rules = app.rules
...
def get_rules_list(app):
	rules = app.rules
	rules_box = rules.children[0]
	return rules_box.ids.ruleslist


def get_rules_view(app):
	rules = app.rules
	rules_box = rules.children[0]
	return rules_box.ids.rulesview


@pytest.fixture(params=[("graphboards", "physical", "pawn", "kobold")])
def kobold_board_selection(request, rule_builder_app, kobold_state):
	app = rule_builder_app
	screen = app.mainscreen
	base = getattr(screen, request.param[0])
	board = base[request.param[1]]
	if len(request.param) > 2:
		mapp = getattr(board, request.param[2])
		if len(request.param) > 3:
			idle_until(
				lambda: request.param[3] in mapp,
				100,
				f"Never got the {request.param[2]} for the {request.param[3]}",
			)
			selection = mapp[request.param[3]]
		else:
			selection = mapp
	else:
		selection = board
...
def test_rule_builder_display_trigger(
	rule_builder_app, kobold_board_selection
):
	rules_list = get_rules_list(rule_builder_app)
	rules_view = get_rules_view(rule_builder_app)
	idle_until(
		lambda: "shrubsprint"
...
def test_rule_builder_remove_trigger(rule_builder_app, kobold_board_selection):
	rules_list = get_rules_list(rule_builder_app)
	rules_view = get_rules_view(rule_builder_app)
	idle_until(
		lambda: "shrubsprint"
	...
	idle_until(
		lambda: not any(
			...
			for func in rule_builder_app.engine.rule["shrubsprint"].triggers
		),
		100,
...
def test_rule_builder_add_trigger(rule_builder_app, kobold_board_selection):
	rules_list = get_rules_list(rule_builder_app)
	rules_view = get_rules_view(rule_builder_app)
	idle_until(
		lambda: "shrubsprint"
...
@pytest.fixture
def polygons_state(play_dir):
	with Engine(play_dir, workers=0) as eng:
		polygons.install(eng)


@pytest.fixture(params=["triangle"])
def polygons_character_selection(request, polygons_state, rule_builder_app):
	app = rule_builder_app
	idle_until(lambda: hasattr(app, "engine"), 100, "App never made engine")
	idle_until(
		lambda: request.param in app.engine.character,
		100,
		f"Engine proxy never made {request.param} character proxy",
	)
	app.select_character(app.engine.character[request.param])
	idle_until(
		lambda: app.character_name == request.param,
		100,
...
def test_char_rule_builder_remove_unit_trigger(
	polygons_character_selection, rule_builder_app
):
	app = rule_builder_app
	idle_until(
		lambda: getattr(app.charrules, "_finalized", False),
