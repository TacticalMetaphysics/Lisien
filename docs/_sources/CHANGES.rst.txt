2026-02-16 (0.23.3)
-------------------

- Fixed the Android build by removing its service worker. Android's service workers really aren't very good for parallelism, and seem to have gotten even worse in recent updates. Thankfully, as of Python 3.14, we don't need them anymore! Running Lisien in a thread, and its workers in subinterpreters, works fine.

- Moved the ``db``, ``sql`` and ``pqdb`` submodules to the ``db`` package.

- The ``time`` property of ``Engine`` acts much more like a normal tuple now. It's hashable, and if you hold onto one, it doesn't change when you time travel. You get a new copy each time you access it from ``Engine``. You can still register functions to be called when the time changes, using ``@engine.time.connect``, as before.


2026-02-07 (0.23.2)
-------------------

- Converted most of the codebase to ``attrs``, which will make initialization easier to understand.

- Removed ``reslot``, because ``attrs`` already does that.

- ``Engine`` defaults to database-free operation if you don't give it a directory to work in.

- The ``export`` method of ``Engine`` now accepts a directory for its second argument, and will use the name of the game as the base name of the archive by default.

- Elide will now export games to *its* working directory, which may be different from the directory it was launched in.

- Fixed crash when resuming a game where rules had run upon ``Thing`` objects.

- Fixed various crashes when sending some object types between subinterpreters. This would mainly be a problem on Android, where you have to use subinterpreters to run code parallel.


2025-11-16 (0.23.1)
-------------------

- Fixed an inconsistency in instantiating ParquetDB that was probably preventing Elide from launching for anybody who wasn't running the unreleased main branch of ParquetDB. Sorry!

- Fixed creating and deleting edges on the database

- Fixed some crashes when trying to make Thing objects travel

- More type safety

- The database connector is now the ``db`` property of the ``Engine`` object (formerly the ``query`` property)

- The hashes in XML exports no longer vary depending on docstrings, formatting, or type annotations. They're not used for anything yet; at some point, the import feature will check the hashes to make sure you have compatible code.


2025-11-13 (0.23.0)
-------------------

- Added support for subinterpreter-based parallelism on Python 3.14, which is now the version used for Android.

- New dependency ``u-msgpack-python`` is used in those subinterpreters instead of the basic ``msgpack``, which doesn't run in subinterpreters (yet).

- Refactored the codebase to use modern type hints; dropped support for Python versions before 3.12. The database schema is now generated dynamically from those annotations, which makes it easier to maintain. The import and export methods now live on the database connectors, plus a couple on ``Engine``.

- Improved the database-free mode of operation that you get when you instantiate ``Engine`` with a prefix of ``None``. Now, you can export XML from the engine when it's run that way. It may become the default mode of operation in a future release.

- Fixed data loss when you close the engine right after creating a graph.

- XML exports now group the changes to the world state by which rule caused them. This enables the rule stepper to work with those exports.

- New dependency ``reslot``, maintained by the author, will reduce Lisien's memory footprint when there are a lot of entities active at once.

- Dropped the ``astor`` dependency, which is unmaintained and not compatible with Python 3.14.


2025-10-11 (0.22.5)
-------------------

- Fixed some data loss when exporting a game without simulating anything on ParquetDB. It made the .lisien files on the itch page kind of weird.


2025-10-03 (0.22.4)
-------------------

- Added the Method and Function code stores to the Python editor in Elide

- Switched from astunparse to astor The astunparse package isn't maintained anymore. astor supports canonicalizing code for comparison.

- Fixed a crash when quitting out of Elide without opening a game


2025-09-28 (0.22.3)
-------------------

- Fixed basic entity creation in Elide, which broke in 0.22.2. Sorry...


2025-09-27 (0.22.2)
-------------------

- Fixed a crash when quitting out of a recently imported game in Elide (meaning the game wouldn't be saved)

- Fix occasional loss of user's Python code, depending on when Elide was closed

- And fix that code sometimes getting a different function name than intended

- Fix saving games using ParquetDB in Elide on desktop


2025-09-22 (0.22.1)
-------------------

- Added an XML schema for the export format.

- Fixed the exporter to actually follow the schema:

- Booleans are now standard "true" and "false", not "T" or "F"

- Base64 hashes use the standard character set

- Fixed the importer to accept compliant world.xml. Noncompliant XML from the 0.22.0 release will still work.


2025-09-19 (0.22.0)
-------------------

- Switch to a new XML-based export format, with .lisien file extension. They're just zip files, like earlier versions' exports, but now you can open the file world.xml in a text editor and change the history of the game, if you want. This improves compatibility between desktop and Android, but breaks compatibility with previous versions, including games that haven't been exported, because of the following item...

- Ordered all database tables chronologically.

- Made the rules engine deterministic. It wasn't consistent about what order rules were applied to many entities, but now it is.


2025-09-04 (0.21.8)
-------------------

- Fix the import and load game functions on Android *again*. They weren't consistently respecting an Elide session running somewhere other than the working directory.


2025-09-03 (0.21.7)
-------------------

- Fix imported games on Android being extracted to a different directory than what Elide opens, resulting in "imported" games being empty all the time, or, if they shared a name with an existing game, you'd load into that game instead.

- Rearranged the ↑ and ↓ buttons in the rule builder.

- Tidied the initial connection to the database a bit. This seemed to fix crashes on starting the game on Android, but I'm not sure why.


2025-08-28 (0.21.6)
-------------------

- Fix Elide's rule builder not showing the cards


2025-08-27 (0.21.5)
-------------------

- Fix "New game" button in a different way

- Fix a crash that happened when a Lisien engine created state, advanced time, and then closed or otherwise keyframed


2025-08-27 (0.21.4)
-------------------

- Fix an issue causing some valid exported games not to import right in Elide, causing an empty map and a freeze

- Add ↑ and ↓ buttons to rules in Elide, for you to reorder them


2025-08-21 (0.21.3)
-------------------

- Fix Elide's session management; exiting and resuming a game weren't always really doing that.

- Removed the remainder of the parallelism on Android, because that was causing issues with starting the game. I'll try again once python-for-android can deploy Python 3.14 and its new ``concurrent.interpreters`` module.


2025-08-13 (0.21.2)
-------------------

- Fix a packaging issue that was causing Elide's main menu not to show up. 0.21.1 was meant to fix the same, but didn't really.


2025-08-10 (0.21.0)
-------------------

- Added the ``bookmark`` mapping/method. This is a convenient way to store points in time and travel to them. You can call it with whatever name you want to give your bookmark to make a new one, then call it with that name again to travel back to that time. Bookmarks are not versioned. If you want a point in time that changes in response to time travel, store the time in the engine's ``universal`` mapping.

- Added support for storing the value ``None``. Values deleted from dictionaries are now represented by the ellipsis object.

- Renamed the ``user`` property of nodes to ``leader``.

- Removed what remained of multigraphs, which may never be supported. This changed the database schema, so old playthroughs are incompatible.

- Generally decrufted the codebase.


2025-08-03 (0.20.2)
-------------------

- Overhauled the internal ``WindowDict`` class and its relatives, ruling out a bunch of race conditions that caused intermittent crashes

- Made deltas behave more consistently, ruling out some types of data loss

- Overhauled the tracking of units and users

- Made entity deletion *really* atomic, this time

- Fixed keyframe snaps sometimes happening after a change had been recorded to the database, but before it had been cached, resulting in the loss of a single change to the game state

- Fixed deleting keyframes


2025-07-17 (0.20.1)
-------------------

- Unify the fast delta code, fixing some changes getting left out of deltas

- Fix tracking of units' users

- Finish up the proxy interface

- Fix setting mutable values (like list and dict) in the ``universal`` mapping

- Fix tracking which rules were already handled

- Fix intermittent crash from attempting to serialize Things whose locations aren't available


2025-07-07 (0.20.0)
-------------------

- Android support. Currently, the Android version supports only SQLite, not ParquetDB. Parallelism is disabled for now, too, because it increases the startup time too much.

- A main menu for Elide. If you don't want it, specify the path to your game folder on the command line, and Elide will launch straight into that game.

- Support Python 3.13.

- Crash fixes

- When resuming a game that was closed at the very beginning of a branch

- Intermittently when deleting entities, if an automatic keyframe snap happened to occur mid-deletion. Fixed by making deletion atomic.


2025-05-23 (0.19.3)
-------------------

- Fix keyframes sometimes getting made every tick, if the automatic keyframe creation was overridden

- Require ParquetDB version 1.0.1, fix some freezes that could occur when loading empty ParquetDB tables and a data integrity issue with the rules- handledness tracking

- Performance improvements when using plans with ParquetDB

- Fixed worker processes sometimes staying open when there's nothing left to do


2025-04-26 (0.19.2)
-------------------

- Added the option to run Lisien without connecting to any database. This is mainly useful for testing. Instantiate the engine like ``Engine(None)`` to use.

- Rewrote the string store, ``Engine.string``, to save any changes you make while the engine is running, like it was supposed to be doing. Use it like a dictionary with both the keys and the values being strings, and set its ``language`` property to change to another language. These strings don't react to time travel at all; it's just a convenient interface to some JSON files.

- More refactoring.


2025-04-19 (0.19.1)
-------------------

- Fixed big rules not resetting the time at the end of a plan block

- Fixed character rulebooks becoming unset after some time travel operations

- Fixed picking a starting directory in Elide

- Removed ``threaded_triggers`` option; triggers are now serial when there are no workers, parallel if there are workers to go parallel with. I plan to make a kind of worker "process" that's really a thread.

- Fixed loading node and portal rulebooks from keyframes

- Fixed loading things' locations from keyframes


2025-04-13 (0.19.0)
-------------------

- Core:

- Switched the default storage backend to ParquetDB, which is far more space-efficient. Use the ``connect_string`` parameter if you want a SQL database, which is still required for historical queries (for now).

- The engine's ``time`` property now includes the tick, as well as the branch and turn.

- Added the ability to record "replays" of Elide sessions. They run the same commands on the Lisien core that Elide would. You won't really be able to play them back, exactly, but if you set a replayfile in elide.ini's [lisien] section, then send the file to me, I'll be able to reproduce whatever bug you ran into.

- When using the proxy interface, you can register functions to be called whenever the time changes, as ``engine_proxy.time.connect(function)``; or whenever the rules engine has simulated a turn, as ``engine_proxy.next_turn.connect(function)``. Also available in the core proper.

- Filled in some missing parts of the proxy API.

- Fixed inconsistent handling of planned changes.

- Fixed some intermittent interprocess sync issues that could result in crashes or freezes.

- Database integrity issues, too.

- Fixed Thing objects sometimes hanging around after deletion, depending on when you closed the engine.

- The old allegedb submodule no longer exists. Its functionality is now part of Lisien proper. Elide frontend:

- Fixed stats running off the left of the window.

- Fixed the grid view not being visible.

- Fixed configuration of the stat widgets.


2025-02-23 (0.18.3)
-------------------

- Fix major memory leaks


2025-02-20 (0.18.2)
-------------------

- A crash fix relating to the unloading code, which was too eager


2025-02-14 (0.18.1)
-------------------

- Crash fixes


2025-02-13 (0.18.0)
-------------------

- Changed name of "LiSE" to "Lisien" -- thanks to https://webb.page for the name

- Changed name of "ELiDE" to "Elide" (letters cased like normal English if that's what you're writing, "elide" and "lisien" in Python)

- Rules may now be designated "big," which will enable an optimization to only write the changes they make to the world after they're all computed.

- Neighborhoods get keyframed properly, now, the same as other rule attributes.

- Added Kenney graphics, and changed the default background to Kenney's parchment

- Fixed rules displaying out of order in the stepper

- Fixed dragged objects dropping when they shouldn't

- Improved locking for caches, fixing some intermittent data loss

- Dropped support for Python 3.8 and 3.9


2024-11-30 (0.17.2)
-------------------

- Fix rule poller. It only worked for some rulebook types before.

- Performance improvements as a result of new caching behavior for the above.


2024-11-09 (0.17.1)
-------------------

- Disable editing all throughout ELiDE whenever the simulation is running. It was only disabled sometimes, in some parts of the UI.

- Add a button to edit a function card without backing out of the rule editor

- Fix ELiDE crash in Python 3.8 when making more than one new Place or Thing


2024-10-31 (0.17.0)
-------------------

- Overhaul the entire caching infrastructure, which should greatly cut down on crashes. No new features, but it does make some changes to the database schema.


2024-10-18 (0.16.16)
--------------------

- Fix pawn duplication in ELiDE


2024-10-16 (0.16.15)
--------------------

- Fix a different way pawns could disappear in ELiDE


2024-10-16 (0.16.14)
--------------------

- Fix disappearing pawns in ELiDE


2024-10-16 (0.16.13)
--------------------

- Fix worker process states getting desynchronized when you made a new character

- Fix methods not being callable in worker processes

- Improved performance when planning a lot of future state

- Made the LiSE engine itself an Executor, which I guess breaks compatibility. Whatever, zerover makes no promises. Use ``Engine.submit(..)`` to schedule computation in worker processes now.

- Made ELiDE's stat list scrollable


2024-10-14 (0.16.2 & 0.16.12)
-----------------------------

- Fix ELiDE crash when adding graph-shaped world state to a grid-shaped world


2024-10-13 (0.16.1)
-------------------

- Fix some incoherent world state that could result from doing things on tick zero of turn zero, if you started the world with a NetworkX graph.

- Fix a rule running when it shouldn't have in the wolfsheep example


2024-10-11 (0.16.0)
-------------------

- A process pool. When you've stored a function in LiSE, you can ``submit`` it to ``Engine.pool.submit(..)`` using the standard Python ``concurrent.futures.Executor`` API. The worker processes all have read-only views on the state of the simulation.

- Trigger functions now run in parallel by default. Set the ``workers`` option to 0 to disable this.

- Neighborhoods. If your rule only runs when something changes near the Thing or Place it runs on, you can indicate this by setting the rule's neighborhood property to an integer, and your sim will run faster. See "Getting started" in the README for an example.

- ELiDE once again updates its graphics in response to events in the LiSE core -- set the "_image_paths" of a ``Thing`` or ``Place`` object to a list of paths.


2024-09-01 (0.15.9)
-------------------

- More data corruption fixes


2024-09-01 (0.15.8)
-------------------

- More concurrency fixes


2024-08-27 (0.15.7)
-------------------

- A variety of database corruption fixes


2024-08-16 (0.15.6)
-------------------

- Fix Thing objects occasionally forgetting their location


2024-08-10 (0.15.5)
-------------------

- Fix a crash when resuming ELiDE in the past of a simulation

- Fix rules sometimes not triggering, based on when the trigger was set


2024-08-08 (0.15.4)
-------------------

- Fix time travel via rule stepper

- Restore the ability to delete Characters

- Even faster interbranch time travel

- Fix a crash when time traveling between times with no changes to the world


2024-07-22 (0.15.3)
-------------------

- More crash fixes


2024-07-05 (0.15.2)
-------------------

- Crash fixes involving time travel

- Speed up interbranch time travel somewhat

- Switch to ruff formatter


2024-06-24 (0.15.1)
-------------------

- Fix another concurrency issue


2024-06-18 (0.15.0)
-------------------

- Automatically unload history, thus minimizing memory usage for long games By default, this happens whenever you commit changes to the database.

- Improved support for ELiDE widgets in games

- New example sim, awareness.py, to demonstrate this

- Moved ``game_start`` into its own module

- Dropped support for Python 3.7 and the lz4 module

- A lot of fixes regarding thread safety and interprocess sync


2023-04-27 (0.14.2)
-------------------

- Fix the rest of the loader


2023-04-26 (0.14.1)
-------------------

- Fix a loading error when you make two keyframes and then start the engine before the first


2023-04-07 (0.14.0)
-------------------

- Seek bar for time travel in ELiDE, and some other minor UI improvements

- Vastly improved performance in ELiDE for large worlds

- History queries no longer require the whole of history to be loaded, and are faster

- SQLAlchemy is now a hard dependency

- numpy is required again

- Save the state of the randomizer every time you use it, not just on time travel

- Rule stepper now works when you reopen the sim

- New example sim, wolfsheep.py

- ``enforce_end_of_time`` option, so it's not so trivial to make voids of time by accident

- Lots of crash fixes

- yapf formatting, with tabs

- Improved documentation


2022-07-31 (0.13.2)
-------------------

- Fix an intermittent error where entity deletion sometimes wouldn't go through all the way

- Performance improvements

- LiSE now saves keyframes on close by default. Change ```keyframe_on_close```` to ````False``` to disable this

- Renamed "modulus" options to end with "interval" instead

- Disable the cache arranger by default


2022-07-23 (0.13.0)
-------------------

- ELiDE now has the Timestream view, easing time travel when history gets complicated

- Rename "avatar" to "unit"

- Nodes now support the ``.user.only`` property, just like characters' ``.unit.only``

- Improve interprocess communication some more, particularly when time travelling between branches

- Tab indentation, with yapf formatting

- Rename some Engine methods to use snake_case

- numpy is no longer required for the LiSE core (will speed up IPC, though)

- Fix a loading error with changes in branches other than trunk

- Crash fixes

- Drop support for Python 3.6


2021-07-04 (0.12.1)
-------------------

- Optimize interprocess synchronization

- Fix some inconsistencies in the rulebook sync

- Crush all of a character's initial state into the one keyframe, fixing integrity problems

- License change; no longer accept later versions of the AGPL

- Support networkx 2.5

- ELiDE changes

- Rule stepper to help you view the state of the game after one rule's run, just before the next

- Don't crash when resuming the game on non-trunk branches

- Fix stats configurator

- Fix rules configurators

- Accept a prefix on the command line

- Grid view shows characters other than the one you started with

- Dialog options from the game itself don't appear more than requested

- Fix crash when laying out one-dimensional boards


2021-04-19 (0.11.2)
-------------------

- Fix the branch input in ELiDE

- Performance improvements


2021-03-17 (0.11.1)
-------------------

- Fix a flaw in the keyframes implementation that caused them to overwrite each other when several were made in a single session


2021-02-03 (0.11)
-----------------

- Add support for "keyframes," snapshots of the world state at some specific time. These are currently most useful for supplying the world's initial state by instantiating a Character with a networkx graph, eg. ``engine.new_character('new', nx.grid_2d_graph(3, 3))``

- Drastically improved load times for large initial world states

- Removed the would-be world generation API; for now, use [the networkx graph generators](https://networkx.org/documentation/stable/reference/generators.html), and ``LiSE.character.grid_2d_8graph``if you want a square grid that's connected at the corners as well as the sides.

- Removed the "sense" feature. It will need a reimagining at some point. In the meantime you can still use ``Character.facade()`` to get a transient, mutable view onto a ``Character`` that won't be saved

- ``allegedb`` is a submodule of ``LiSE`` now, and will not be published on its own.

- Dropped support for graph types other than ``DiGraph``

- Dropped support for Python versions prior to 3.6

- Compatibility with msgpack 1.0.0

- ELiDE changes

- Grid view

- File browser

- Grid generator when starting new simulation

- Symmetrical portals

- New agenda widget, for scheduling things in the future The stat list on the left of the default ELiDE view is technically a calendar now, where an agenda is a related widget class that offers more flexibility to interface designers.

- Require Kivy 2.0


2020-01-08 (0.10.1)
-------------------

- Compatibility with networkx 2.4

- Smaller database

- Performance improvements

- Default database filename is now 'world.db', and you can instantiate Engine with no arguments


2019-03-16 (0.10)
-----------------

- Improved documentation.

- Improved performance.

- Prevented the user from trying to start the rules engine while it's already running.

- Minor API changes. clear_world and clear_code are now one option, clear

- Any trigger that's going to run in a given turn is now guaranteed to run prior to any prereq or action. This is mostly to prepare for experiments in parallelism.

- If you branch the timeline while stuff is planned for the future, the plan will be copied to the new branch, unless the plan has not yet started.

- Made facades act more like regular characters. They still aren't very useful... I'm planning to add some new API to them that lets you munge and filter their data lazily


2018-10-29 (0.9)
----------------

- This release is for ProcJam 2018.

- Changed license to AGPL3+, but also waive rights in case of my death.

- Switched to semantic versioning.

- Made the frontend experience faster by implementing deltas. Use Engine.get_delta(...) to rapidly generate a dictionary describing changes between two points in time in a single branch.

- Switched serialization format from an abuse of JSON to msgpack.

- Improved the API of the ELiDE widgets, making it easier to use them in your own game. See https://github.com/LogicalDash/DungeonUniversity for an example.

- You can zoom the map with the mousewheel now.

- Reduced memory usage for large worlds. They still take a long time to load; I'll work on that.

- Wrote a real tutorial for building LiSE games in Python outside of ELiDE.

- Made the order of execution for rulebooks easier to understand. Basically, rulebooks are followed in alphabetical order; rules within them go in whatever order they were added.

- Rearranged the UI a little.

- You can edit every rulebook in ELiDE now, including the ones that apply to all a character's avatars and so forth.

- Rules will only be followed for entities that still exist when they're run.

- The turn will always increment *before* running any rules, and no rules will be run on turn 0.

- Various little performance improvements and fixes to things like data integrity.


2017-10-06 (Alpha 8)
--------------------

- This release is for ProcJam 2017.

- Made the time model more granular. What was formerly called a tick is now a turn, which contains as many ticks as you need.

- Support a rudimentary form of dialog menu for player interaction.

- Rules and rulebooks are now journaled, and may change throughout the game.

- Python source code is no longer stored in SQLite--just plain text files.

- Changed the rule API so that rule functions are only ever called with a single argument, the entity that they apply to.

- ELiDE is now a pure Python module.

- Performance improvements.


2017-05-06 (Alpha 7)
--------------------

- Restructure everything in LiSE around a new caching strategy.

- Generally cleaner UI.

- If all your places are named with pairs of integers, you get automatic grid layout in ELiDE.


2016-02-01 (Alpha 6)
--------------------

- Opening a functions editor no longer crashes the app.

- All the function editors are now in tabs of one screen.

- Nicer auto layout of spots.

- Various smaller fixes.


2016-01-03 (Alpha 5)
--------------------

- A whole new approach to interprocess communication resulting in much more acceptable performance within ELiDE.

- LiSE caches the whole world state.

- ELiDE has a new caching scheme as well. This is invisible to the user but rules out some aliasing bugs.

- Fixed some rules not being polled.

- A new example, college.py, and some fixes to kobold.py where it hadn't kept up with API changes.

- The first sprouts of a world generation API, currently undocumented since I myself don't know how to work with it. Observe its methods within the class AbstractCharacter in the LiSE.character module. Chain the methods together and play around with the order to the best effect.

- Lots of refactoring.

- Some new graphics from Marsh Davies for Procjam 2015. And it's easier to include your own now, with an atlas and a change in elide.ini (though undocumented).

- Real version numbers in the Python packages.


2015-08-26 (Alpha 4)
--------------------

- The rules editor is now usable under normal conditions. After you've made Actions and Triggers, check this out by selecting something and pressing the Rules button.

- The big > button will cause time to pass for so long as it is toggled, as it was always meant to.

- Improvements to input handling and collision detection.

- Less cryptic README.

- Crash fixes.


2015-08-11 (Alpha 3)
--------------------

- Performance improvements.

- Architectural changes in ELiDE.

- Many crash bugs fixed.

- Pop-over windows changed into Screens, making them tidier and enabling transition animations that aren't much to look at presently.

- Thing and Place graphics can now be added in the elide.ini configuration file.


2015-03-29 (Alpha 2)
--------------------

- The core LiSE simulation process and the ELiDE graphical frontend are now running in separate processes.

- Fixed some UI interactions.


2015-03-06 (Alpha 1)
--------------------

- Initial release.