...
from .util import AbstractCharacter, AbstractEngine, getatt, singleton_get, timer
...
class FacadeEntity(MutableMapping, Signal, ABC):
    ...
    def __init__(self, mapping, _=None, **kwargs):
        ...
        self._real = mapping
        self._patch = {
            k: v.unwrap() if hasattr(v, "unwrap") else v for (k, v) in kwargs.items()
        }
...
class FacadeThing(FacadeNode):
    def __init__(self, mapping, real_or_name, **kwargs):
        ...
        if location is None and not (
            isinstance(real_or_name, Thing) or isinstance(real_or_name, FacadeThing)
        ):
            raise TypeError(
        ...
        self._real = {
            "name": (
                real_or_name.name if hasattr(real_or_name, "name") else real_or_name
            ),
            "location": location,
        }
...
class Character(DiGraph, AbstractCharacter, RuleFollower):
    ...
    * ``adj.rule`` to make a rule run on all the edges this Character
    ...
    def __bool__(self):
        try:
            return (
                self.engine._graph_cache.retrieve(self.name, *self.engine._btt())
                != "Deleted"
            )
        except KeyError:
            return False
    ...
    class ThingMapping(MutableMappingUnwrapper, RuleFollower, Signal):
        ...
        def __setitem__(self, thing, val):
            ...
            self.engine._set_thing_loc(self.character.name, thing, val["location"])
    ...
    class PlaceMapping(MutableMappingUnwrapper, RuleFollower, Signal):
        ...
        def __getitem__(self, place):
            ...
            branch, turn, tick = btt()
            if not nodes_contains(charn, place, branch, turn, tick) or things_contains(
                charn, place, branch, turn, tick
            ):
    ...
    class PortalSuccessorsMapping(DiGraphSuccessorsMapping, RuleFollower):
        ...
        def update(self, other, **kwargs):
            ...
            tick = start_tick + 1
            with timer("seconds spent updating PortalSuccessorsMapping", engine.debug):
                for orig, dests in chain(other.items(), kwargs.items()):
                    for dest, kvs in dests.items():
                        if kvs is None:
                            ...
                            exist_edge(charn, orig, dest, 0, branch, turn, tick, False)
                            tick += 1
                        else:
                            store_edge(
                                charn,
                            ...
                            exist_edge(charn, orig, dest, 0, branch, turn, tick, True)
                            tick += 1
                            for k, v in kvs.items():
                                ...
                                tick += 1
            parent, start_turn, start_tick, end_turn, _ = self.engine._branches[branch]
        ...
        class Successors(DiGraphSuccessorsMapping.Successors):
            ...
            def __setitem__(self, dest, value):
                ...
                for k, v in value.items():
                    ...
                    db_edge_val_set(charn, orig, dest, 0, k, branch, turn, tick, v)
                    edge_val_cache_store(charn, orig, dest, 0, k, branch, turn, tick, v)
            ...
            def update(self, other, **kwargs):
                ...
                if (
                    ...
                    raise RuntimeError("Tried to update successors in the past")
                if (
                    turn > engine._branches[branch][3]
                    or start_tick > engine._turn_end_plan[branch, turn]
                ):
                    raise RuntimeError("Tried to update successors in the future")
                tick = start_tick + 1
                with self.db.world_lock:
                    for dest, val in chain(other.items(), kwargs.items()):
                        if val is None:
                            ...
                            exist_edge(charn, orig, dest, 0, branch, turn, tick, None)
                            tick += 1
                        else:
                            store_edge(
                                charn,
                            ...
                            for key, value in val.items():
                                ...
                                tick += 1
                    parent, start_turn, start_tick, end_turn, _ = self.db._branches[
                        branch
                    ]
    ...
    class UnitGraphMapping(Mapping, RuleFollower):
        ...
        def node(self):
            ...
            try:
                ...
                raise AttributeError("I have no unit, or I have units in many graphs")
    ...
    def add_thing(self, name, location, **kwargs):
        ...
        if name in self.thing:
            raise WorldIntegrityError("Already have a Thing named {}".format(name))
    ...
    def add_unit(self, a, b=None):
        ...
        if self.engine._planning:
            raise NotImplementedError("Currently can't add units within a plan")
        if b is None:
            if not isinstance(a, (Place, Thing)):
                raise TypeError(
                    "when called with one argument, " "it must be a place or thing"
                )
            g = a.character.name
            n = a.name
        else:
        ...
        branch, turn, tick = self.engine._nbtt()
        self.engine._unitness_cache.store(self.name, g, n, branch, turn, tick, True)
        self.engine.query.unit_set(self.name, g, n, branch, turn, tick, True)

    def remove_unit(self, a, b=None):
        """This is no longer my unit, though it still exists"""
        if self.engine._planning:
            raise NotImplementedError("Currently can't remove units within a plan")
        if b is None:
        ...
        branch, turn, tick = self.engine._nbtt()
        self.engine._unitness_cache.store(self.name, g, n, branch, turn, tick, False)
