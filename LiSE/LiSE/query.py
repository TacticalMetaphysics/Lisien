    ...
    if left == right:
        ...
        if right[0] is None or right[0] == (None, None):
            ...
            return max((left[0], right[0])), (
                (None, None) if isinstance(left[0], tuple) else None
            )
        elif left[0] <= right[1]:
            return left[0], right[1]
        else:
...
def _the_select(tab: Table, val_col="value"):
    return select(
        ...
        func.lead(tab.c.turn).over(order_by=(tab.c.turn, tab.c.tick)).label("turn_to"),
        func.lead(tab.c.tick).over(order_by=(tab.c.turn, tab.c.tick)).label("tick_to"),
        tab.c[val_col],
    )
...
def _make_side_sel(entity, stat, branches: List[str], pack: callable, mid_turn: bool):
    from .character import AbstractCharacter
    from .node import Place, Thing
    from .portal import Portal

    if isinstance(entity, AbstractCharacter):
        return _make_graph_val_select(pack(entity.name), pack(stat), branches, mid_turn)
...
class QueryResultMidTurn(QueryResult):
    def _generate(self):
        ...
        for span, buul in zip(spans, bools):
            if buul:
                for turn in range(span[0][0], span[1][0] + (1 if span[1][1] else 0)):
                    if turn in trues:
                        continue
                    trues.add(turn)
                    _list.append(turn)
...
class QueryEngine(query.QueryEngine):
    ...
    def __init__(self, dbstring, connect_args, pack=None, unpack=None):
        super().__init__(dbstring, connect_args, pack, unpack, gather=gather_sql)
    ...
    def _get_one_window(self, ret, branch, turn_from, tick_from, turn_to, tick_to):
        super()._get_one_window(ret, branch, turn_from, tick_from, turn_to, tick_to)
        unpack = self.unpack
        ...
        while isinstance(got := self._outq.get(), list):
            for graph, node, turn, tick, loc in got:
                ...
                ret[graph]["things"].append((graph, node, branch, turn, tick, loc))
        assert got == (
            "end",
            "things",
            branch,
        ...
        while isinstance(got := self._outq.get(), list):
            for graph, turn, tick, rb in got:
                ...
                ret[graph]["character_rulebook"].append((graph, branch, turn, tick, rb))
        assert got == (
            "end",
            "character_rulebook",
            branch,
        ...
        while isinstance(got := self._outq.get(), list):
            for graph, turn, tick, rb in got:
                (graph, rb) = map(unpack, (graph, rb))
                ret[graph]["unit_rulebook"].append((graph, branch, turn, tick, rb))
        assert got == (
            "end",
            "unit_rulebook",
            branch,
        ...
        while isinstance(got := self._outq.get(), list):
            for rule, branch, turn, tick, triggers in got:
                ...
                if "rule_triggers" in ret:
                    ret["rule_triggers"].append((rule, branch, turn, tick, triggers))
                else:
                    ret["rule_triggers"] = [(rule, branch, turn, tick, triggers)]
        assert got == (
            "end",
            "rule_triggers",
            branch,
        ...
        while isinstance(got := self._outq.get(), list):
            for rule, branch, turn, tick, prereqs in got:
                ...
                if "rule_prereqs" in ret:
                    ret["rule_prereqs"].append((rule, branch, turn, tick, prereqs))
                else:
        ...
        while isinstance(got := self._outq.get(), list):
            for rule, branch, turn, tick, actions in got:
                ...
                if "rule_actions" in ret:
                    ret["rule_actions"].append((rule, branch, turn, tick, actions))
                else:
    ...
    def edge_val_set(self, graph, orig, dest, idx, key, branch, turn, tick, value):
        super().edge_val_set(graph, orig, dest, idx, key, branch, turn, tick, value)
        self._increc()
    ...
    def _rule_dump(self, typ):
        unpack = self.unpack
        for rule, branch, turn, tick, lst in self.call_one("rule_{}_dump".format(typ)):
    ...
    def _set_rule_something(self, what, rule, branch, turn, tick, flist):
        ...
        self.call_one("rule_{}_insert".format(what), rule, branch, turn, tick, flist)
        self._increc()
    ...
    def set_rulebook(self, name, branch, turn, tick, rules=None, prio=0.0):
        ...
        self.call_one("rulebooks_insert", name, branch, turn, tick, rules, float(prio))
        self._increc()
    ...
    set_unit_rulebook = partialmethod(_set_rulebook_on_character, "unit_rulebook")
    ...
    def set_node_rulebook(self, character, node, branch, turn, tick, rulebook):
        (character, node, rulebook) = map(self.pack, (character, node, rulebook))
        self.call_one(
    ...
    def handled_character_rule(self, character, rulebook, rule, branch, turn, tick):
        (character, rulebook) = map(self.pack, (character, rulebook))
        self._char_rules_handled.append((character, rulebook, rule, branch, turn, tick))
        self._increc()

    def _flush_char_rules_handled(self):
        if not self._char_rules_handled:
            return
        self.call_many("character_rules_handled_insert", self._char_rules_handled)
        self._char_rules_handled = []
    ...
    def handled_character_thing_rule(
        ...
        character, thing, rulebook = map(self.pack, (character, thing, rulebook))
    ...
    def handled_character_place_rule(
        ...
        character, rulebook, place = map(self.pack, (character, rulebook, place))
    ...
    def handled_node_rule(self, character, node, rulebook, rule, branch, turn, tick):
        (character, node, rulebook) = map(self.pack, (character, node, rulebook))
    ...
    def _flush_portal_rules_handled(self):
        if not self._portal_rules_handled:
            return
        self.call_many("portal_rules_handled_insert", self._portal_rules_handled)
        self._portal_rules_handled = []

    def get_rulebook_char(self, rulemap, character):
        character = self.pack(character)
        for (book,) in self.call_one("rulebook_get_{}".format(rulemap), character):
    ...
    def unit_set(self, character, graph, node, branch, turn, tick, isav):
        ...
        self._unitness.append((character, graph, node, branch, turn, tick, isav))
        self._increc()
    ...
    def rulebook_get(self, rulebook, idx):
        return self.unpack(
            self.call_one("rulebook_get", self.pack(rulebook), idx).fetchone()[0]
        )
    ...
    def rulebook_set(self, rulebook, branch, turn, tick, rules):
        ...
        try:
            self.call_one("rulebooks_insert", rulebook, branch, turn, tick, rules)
            self._increc()
        except IntegrityError:
            self.call_one("rulebooks_update", rules, rulebook, branch, turn, tick)
