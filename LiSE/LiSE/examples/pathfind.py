...
def install(eng, seed=None):
    ...
    def find_path_somewhere(node):
        ...
        node.engine.debug(f"{node.name}'s shortest path to {destx, desty} is {ret}")
        return ret
    ...
    def go_places(char):
        ...
        def log_as_completed(fut):
            try:
                char.engine.debug(f"Got path for {fut.thing.name}: {fut.result()}")
            except NetworkXNoPath:
                char.engine.debug(f"No path for {fut.thing.name}")

        futs = []
        for thing in char.thing.values():
            fut = char.engine.submit(char.engine.function.find_path_somewhere, thing)
            fut.thing = thing
