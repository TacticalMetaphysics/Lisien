...
def install(
    ...
    def dieoff(critter):
        ret = (
            "malaria" if not (critter["sickle_a"] or critter["sickle_b"]) else "anemia"
        )
        critter.delete()
    ...
    def mate(critter):
        ...
        suitors = list(
            oc for oc in critter.location.contents() if oc["male"] != critter["male"]
        )
        ...
        species.add_unit("physical", name)
        critter["last_mate_turn"] = other_critter["last_mate_turn"] = engine.turn
        return "mated"
    ...
    def not_travelling(critter):
        return (
            "destination" not in critter or critter["destination"] == critter.location
        )
