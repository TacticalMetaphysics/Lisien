...
def install(eng: Engine, map_size=(25, 25), wolves=10, sheep=25, seed=None):
    ...
    unoccupied = [(x, y) for x in range(map_size[0]) for y in range(map_size[0])]
    ...
    for i in range(sheep):
        loc = phys.place[unoccupied.pop()]
        shep = loc.new_thing(f"sheep{i}", _image_paths=["atlas://rltiles/dc-mon/sheep"])
    ...
    def graze(shep):
        ...
        shep.engine.character["physical"].stat["bare_places"].append(shep["location"])
    ...
    def pursue_sheep(wolff):
        ...
        sheep_locs = [
            sheep["location"] for sheep in wolff.engine.character["sheep"].units()
        ]
