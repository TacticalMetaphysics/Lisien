...
from . import rule
from .allegedb import HistoricKeyError, Key, graph
...
class UserMapping(Mapping):
    ...
    def _user_names(self) -> Iterator[Key]:
        ...
        for b, r, t in engine._iter_parent_btt():
            for user in engine._unitness_cache.user_cache.iter_keys(charn, nn, b, r, t):
                if user in seen:
                    continue
                seen.add(user)
                try:
...
class Node(graph.Node, rule.RuleFollower):
    ...
    def shortest_path(self, dest: Union[Key, "Node"], weight: Key = None) -> List[Key]:
    ...
    def path_exists(self, dest: Union[Key, "Node"], weight: Key = None) -> bool:
    ...
    def new_portal(self, other: Union[Key, "Node"], **stats) -> "LiSE.portal.Portal":
...
class Thing(Node):
    ...
    def go_to_place(self, place: Union[Node, Key], weight: Key = None) -> int:
        ...
        turns = (
            1
            if weight is None
            else self.engine._portal_objs[(self.character.name, curloc, place)].get(
                weight, 1
            )
        )
    ...
    def follow_path(self, path: list, weight: Key = None, check: bool = True) -> int:
        ...
        with eng.world_lock:
            ...
            for subplace in subpath:
                ...
                _, start_turn, start_tick, end_turn, end_tick = eng._branches[branch]
                if (
                    (start_turn < turn < end_turn)
                    or (
                        start_turn == end_turn == turn and start_tick <= tick < end_tick
                    )
            ...
            with eng.plan():
                for subplace, turn_inc in zip(subpath, turn_incs):
                    ...
                    eng._things_cache.store(charn, myname, branch, turn, tick, subplace)
                    eng.query.set_thing_loc(charn, myname, branch, turn, tick, subplace)
            return turns_total
