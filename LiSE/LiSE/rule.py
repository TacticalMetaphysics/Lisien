...
class RuleFuncList(MutableSequence, Signal, ABC):
    ...
    def __repr__(self):
        return (
            f"<class 'LiSE.rule.{self.__class__.__name__}' [{', '.join(self._get())}]>"
        )
    ...
    * prereqs, which prevent the actions from happening when triggered
    ...
    def __init__(
        ...
        if create:
            ...
            if (
                self.engine._triggers_cache.contains_key(name, branch, turn, tick)
                or self.engine._prereqs_cache.contains_key(name, branch, turn, tick)
                or self.engine._actions_cache.contains_key(name, branch, turn, tick)
            ):
            ...
            if (branch, turn, tick) in self.engine._keyframes_times:
                ...
                try:
                    preqkf = self.engine._prereqs_cache.get_keyframe(branch, turn, tick)
                except KeyError:
                    preqkf = {
                ...
                try:
                    actkf = self.engine._actions_cache.get_keyframe(branch, turn, tick)
                except KeyError:
                    actkf = {
                ...
                preqkf[name] = prereqs
                actkf[name] = actions
                self.engine._triggers_cache.set_keyframe(branch, turn, tick, trigkf)
                self.engine._prereqs_cache.set_keyframe(branch, turn, tick, preqkf)
                self.engine._actions_cache.set_keyframe(branch, turn, tick, actkf)
...
class RuleBook(MutableSequence, Signal):
    ...
    def __setitem__(self, i, v):
        ...
        self.engine.query.set_rulebook(self.name, branch, turn, tick, cache, prio)
...
class AllRules(MutableMapping, Signal):
    ...
    def __setitem__(self, k, v):
        ...
        if callable(v):
            ...
            raise TypeError("Don't know how to store {} as a rule.".format(type(v)))
