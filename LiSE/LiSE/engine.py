...
from .allegedb.cache import KeyframeError, PickyDefaultDict, StructuredDefaultDict
...
from .xcollections import FunctionStore, MethodStore, StringStore, UniversalMapping
...
class Engine(AbstractEngine, gORM, Executor):
        ...
        *,
        ...
        if "rando_state" in self.universal:
            ...
            if self._oturn == self._otick == 0:
                ...
                self.query.universal_set("rando_state", self.branch, 0, 0, rando_state)
            else:
        ...
        if workers > 0:
            ...
            for i in range(workers):
                ...
                logq = Queue()
                logthread = Thread(target=sync_log_forever, args=(logq,), daemon=True)
                proc = Process(
            ...
            self._fut_manager_thread = Thread(target=self._manage_futs, daemon=True)
    ...
    def _call_in_subprocess(
        ...
        argbytes = zlib.compress(self.pack((uid, method, (func_name, *args), kwargs)))
    ...
    def submit(self, fn: Union[FunctionType, MethodType], /, *args, **kwargs) -> Future:
    ...
    def _call_every_subproxy(self, method: str, *args, **kwargs):
        ...
        for _ in range(len(self._worker_processes)):
            ...
            argbytes = zlib.compress(self.pack((self._top_uid, method, args, kwargs)))
    ...
    def _init_graph(
        ...
        if hasattr(data, "stat"):
            ...
            things = {thing: thing["location"] for thing in data.thing.values()}
    ...
    def _load_between(
        ...
        loaded = super()._load_between(branch, turn_from, tick_from, turn_to, tick_to)
        ...
        for graph, rowdict in loaded.items():
            ...
            if rowdict.get("character_rulebook"):
                self._characters_rulebooks_cache.load(rowdict["character_rulebook"])
    ...
    def _load(
        ...
        for loaded_graph, data in loaded.items():
            ...
            if data.get("character_rulebook"):
                self._characters_rulebooks_cache.load(data["character_rulebook"])
    ...
    def _make_node(self, graph: Character, node: Key) -> Union[thing_cls, place_cls]:
    ...
    def _copy_kf(self, branch_from, branch_to, turn, tick):
        ...
        for character in self._graph_cache.iter_entities(branch_from, turn, tick):
            ...
            self._things_cache.set_keyframe((character,), branch_to, turn, tick, loc_kf)
            self._node_contents_cache.set_keyframe(
        ...
        self.query.keyframe_extension_insert(
            ...
            {
                "triggers": self._triggers_cache.get_keyframe(branch_to, turn, tick),
                "prereqs": self._prereqs_cache.get_keyframe(branch_to, turn, tick),
                "actions": self._actions_cache.get_keyframe(branch_to, turn, tick),
            },
    ...
    def _get_kf(self, branch: str, turn: int, tick: int, copy: bool = True) -> dict:
        ...
        kf["universal"] = self._universal_cache.get_keyframe(branch, turn, tick)
    ...
    def _get_keyframe(self, branch: str, turn: int, tick: int, copy=True, silent=False):
        if (branch, turn, tick) in self._keyframes_loaded:
            if silent:
                return
            return self._get_kf(branch, turn, tick, copy=copy)
        univ, rule, rulebook = self.query.get_keyframe_extensions(branch, turn, tick)
        ...
        ret = super()._get_keyframe(branch, turn, tick, copy=copy, silent=False)
            ...
            if graph in ret["node_val"]:
                ...
                for node, val in ret["node_val"][graph].items():
                    ...
                    if location in conts:
                        ...
                        conts[location] = {node}
                self._things_cache.set_keyframe((graph,), branch, turn, tick, locs)
                self._node_contents_cache.set_keyframe(
                    (graph,),
                    branch,
                    turn,
                ...
                self._node_contents_cache.set_keyframe((graph,), branch, turn, tick, {})
        self._characters_rulebooks_cache.set_keyframe(branch, turn, tick, charrbkf)
    ...
    def _is_timespan_too_big(self, branch: str, turn_from: int, turn_to: int) -> bool:
        ...
        * ``'rules'``, a dictionary keyed by the name of each changed rule,
        ...
        if time_from[0] == time_to[0]:
            if slow or self._is_timespan_too_big(
                ...
                return self._get_branch_delta(*time_from, time_to[1], time_to[2])
        return self._unpack_slightly_packed_delta(
            self._get_slow_delta(time_from, time_to)
        )
    ...
    def _get_slow_delta(
        ...
        def newgraph():
            return {
                ...
                NODES: PickyDefaultDict(bytes, args_munger=None, kwargs_munger=None),
                EDGES: PickyDefaultDict(bytes, args_munger=None, kwargs_munger=None),
        ...
        for graph in kf_from["edge_val"].keys() | kf_to["edge_val"].keys():
            ...
            for orig, dest in edges:
                a = kf_from["edge_val"].get(graph, {}).get(orig, {}).get(dest, {})
                b = kf_to["edge_val"].get(graph, {}).get(orig, {}).get(dest, {})
                for k in a.keys() | b.keys():
        ...
        with ThreadPoolExecutor() as pool:
            nodes_intersection = kf_from["nodes"].keys() & kf_to["nodes"].keys()
            deleted_nodes = {}
            for graph in nodes_intersection:
                deleted_nodes_here = deleted_nodes[graph] = (
                    kf_from["nodes"][graph].keys() - kf_to["nodes"][graph].keys()
                )
            ...
            for k, va, vb, _ in filter(
                ...
                futs.append(
                    pool.submit(pack_one, k, va, vb, deleted_nodes, deleted_edges)
                )
            for graf in kf_from["graph_val"].keys() - kf_to["graph_val"].keys():
                delta[self.pack(graf)] = NONE
            for graph in nodes_intersection:
                for node in (
                    kf_to["nodes"][graph].keys() - kf_from["nodes"][graph].keys()
                ):
            ...
            for deleted in kf_from["graph_val"].keys() - kf_to["graph_val"].keys():
    ...
    def _get_branch_delta(
        ...
        charthrbbranches = getattr(self._characters_things_rulebooks_cache, attribute)
        charplrbbranches = getattr(self._characters_places_rulebooks_cache, attribute)
        charporbbranches = getattr(self._characters_portals_rulebooks_cache, attribute)
        ...
        def updav(char, graph, *args):
            ...
            if char in delta and delta[char] is None:
                return
            delta.setdefault(char, {}).setdefault("units", {}).setdefault(graph, {})[
                node
            ] = bool(av)
        ...
        def updnoderb(character, node, rulebook):
            if (character in delta) and (
                ...
                return
            delta.setdefault(character, {}).setdefault("node_val", {}).setdefault(
                node, {}
            )["rulebook"] = rulebook
    ...
    def _get_turn_delta(
        ...
        avatar_rulebooks_settings = getattr(self._units_rulebooks_cache, attribute)
        ...
        node_rulebooks_settings = getattr(self._nodes_rulebooks_cache, attribute)
        portal_rulebooks_settings = getattr(self._portals_rulebooks_cache, attribute)
        if branch in universals_settings and turn in universals_settings[branch]:
            for _, key, val in universals_settings[branch][turn][tick_from:tick_to]:
                delta.setdefault("universal", {})[key] = val
        if branch in unitness_settings and turn in unitness_settings[branch]:
            for chara, graph, *stuff in unitness_settings[branch][turn][
                ...
                for node, is_av in stuff[0].items():
                    chardelt.setdefault("units", {}).setdefault(graph, {})[node] = is_av
        ...
        if (
            ...
            for _, character, rulebook in character_rulebooks_settings[branch][turn][
                tick_from:tick_to
            ]:
        ...
        if (
            ...
            for _, character, rulebook in avatar_rulebooks_settings[branch][turn][
                tick_from:tick_to
            ]:
                chardelt = delta.setdefault(character, {})
                if chardelt is None:
        ...
        if (
            ...
            for _, character, rulebook in character_thing_rulebooks_settings[branch][
                turn
            ][tick_from:tick_to]:
                chardelt = delta.setdefault(character, {})
                if chardelt is None:
        ...
        if (
            ...
            for _, character, rulebook in character_place_rulebooks_settings[branch][
                turn
            ][tick_from:tick_to]:
                chardelt = delta.setdefault(character, {})
                if chardelt is None:
        ...
        if (
            ...
            for _, character, rulebook in character_portal_rulebooks_settings[branch][
                turn
            ][tick_from:tick_to]:
                chardelt = delta.setdefault(character, {})
                if chardelt is None:
        ...
        if (
            ...
            for character, node, rulebook in node_rulebooks_settings[branch][turn][
                tick_from:tick_to
            ]:
                chardelt = delta.setdefault(character, {})
                if chardelt is None:
        ...
        if (
            ...
            for setting in portal_rulebooks_settings[branch][turn][tick_from:tick_to]:
                try:
                ...
                if chardelt is None:
                    continue
                chardelt.setdefault("edge_val", {}).setdefault(orig, {}).setdefault(
                    dest, {}
                )["rulebook"] = rulebook
        return delta
    ...
    def _snap_keyframe_from_delta(
        ...
        for graph in (
            ...
            if delt is None:
                continue
            charunit = self._unitness_cache.get_keyframe((graph,), b, r, t, copy=True)
            ...
            locs = self._things_cache.get_keyframe((graph,), b, r, t, copy=True)
            kf = self._node_contents_cache.get_keyframe((graph,), b, r, t, copy=True)
        ...
        self._characters_rulebooks_cache.set_keyframe(branch, turn, tick, charrbs)
    ...
    def _handled_char(
        ...
        try:
            ...
            return
        self.query.handled_character_rule(charn, rulebook, rulen, branch, turn, tick)
    ...
    def _follow_rules(self):
        ...
        def check_triggers(prio, rulebook, rule, handled_fun, entity, neighbors=None):
        ...
        for (
            ...
            if truthfun in self.rulebook[rulebook]:
                ...
                continue
            trig_futs.append(
                submit(check_triggers, prio, rulebook, rule, handled, entity, None)
            )
        ...
        def get_neighbors(
            ...
            def get_place_neighbors(name: Key) -> Set[Key]:
                seen: Set[Key] = set()
                for succ in self._edges_cache.iter_successors(charn, name, *btt):
                    seen.add(succ)
                for pred in self._edges_cache.iter_predecessors(charn, name, *btt):
                    seen.add(pred)
                return seen

            def get_place_contents(name: Key) -> Set[Key]:
                try:
                    return self._node_contents_cache.retrieve(charn, name, *btt)
                except KeyError:
                    return set()
            ...
            def get_place_portals(name: Key) -> Set[Tuple[Key, Key]]:
                ...
                seen.update(
                    (name, dest)
                    for dest in self._edges_cache.iter_successors(charn, name, *btt)
                )
                seen.update(
                    (orig, name)
                    for orig in self._edges_cache.iter_predecessors(charn, name, *btt)
                )
                return seen
                    ...
                    if len(neighbor) == 2:
                        ...
                        for neighbor_place in chain(
                            ...
                            if neighbor_place not in seen:
                                neighbors.append((neighbor_place,))
                                seen.add(neighbor_place)
                            for neighbor_thing in get_place_contents(neighbor_place):
                                if neighbor_thing not in seen:
                                    neighbors.append((neighbor_thing,))
                                    seen.add(neighbor_thing)
        ...
        for (
            ...
            rulebook,
            rulen,
        ) in self._unit_rules_handled_cache.iter_unhandled_rules(branch, turn, tick):
        ...
        for (
            ...
            if not node_exists(charn, noden):
                continue
            rule = rulemap[rulen]
            handled = partial(handled_node, charn, noden, rulebook, rulen, branch, turn)
        ...
        **kwargs,
        ...
        if layout and data:
            ...
            try:
                ...
                layout = normalize_layout(
                    spring_layout(
                        [name for name, node in nodes.items() if "location" not in node]
                    )
                )
    ...
    def _snap_keyframe_de_novo(self, branch: str, turn: int, tick: int) -> None:
        ...
        for char in all_graphs:
            char_kf = {}
            for graph in self._unitness_cache.iter_keys(char, branch, turn, tick):
                char_kf[graph] = {
            ...
            self._unitness_cache.set_keyframe((char,), branch, turn, tick, char_kf)
        rbnames = list(self._rulebooks_cache.iter_keys(branch, turn, tick))
        rbs = {}
        for rbname in rbnames:
            try:
                rbs[rbname] = self._rulebooks_cache.retrieve(rbname, branch, turn, tick)
            except KeyError:
        ...
        for rule in rulenames:
            try:
                trigs[rule] = self._triggers_cache.retrieve(rule, branch, turn, tick)
            except KeyError:
                trigs[rule] = tuple()
            try:
                preqs[rule] = self._prereqs_cache.retrieve(rule, branch, turn, tick)
            except KeyError:
                preqs[rule] = tuple()
            try:
        ...
        for charname in all_graphs:
            ...
            locs = {}
            conts_mut = {}
            for thingname in self._things_cache.iter_keys(charname, branch, turn, tick):
                try:
            ...
            self._unitness_cache.set_keyframe((charname,), branch, turn, tick, units)
    ...
    def _snap_keyframe_de_novo_graph(
        ...
        self._node_contents_cache.set_keyframe((graph,), branch, turn, tick, contkf)
    ...
    def turns_when(self, qry: Query, mid_turn=False) -> Union[QueryResult, set]:
    ...
    def _node_contents(self, character: Key, node: Key) -> Set:
        return self._node_contents_cache.retrieve(character, node, *self._btt())
    ...
    def apply_choices(
        ...
        for track in choices:
            ...
            if isinstance(permissible, tuple):
                ...
                msg = ""
            if not permissible:
                for turn, changes in enumerate(track["changes"], start=self.turn + 1):
                    rejections.extend(((turn, entity, k, v), msg) for (k, v) in changes)
                continue
            for turn, changes in enumerate(track["changes"], start=self.turn + 1):
                for k, v in changes:
                    ekv = (entity, k, v)
