...
from .util import AbstractCharacter, AbstractEngine, MsgpackExtensionType, getatt
from .xcollections import AbstractLanguageDescriptor, FunctionStore, StringStore
    ...
    def _plain_dest_name(self, dest):
        if isinstance(dest, NodeProxy):
            if dest.character != self.character:
                raise ValueError("{} not in {}".format(dest.name, self.character.name))
            return dest.name
        else:
...
class PortalProxy(CachingEntityProxy):
    ...
    def _get_rulebook_proxy(self):
        return self.engine._char_port_rulebooks_cache[self._charname][self._origin][
            self._destination
        ]
...
class NodeMapProxy(MutableMapping, Signal):
    ...
    def _set_rulebook_proxy(self, rb):
        self.engine._character_rulebooks_cache[self._charname]["node"] = RuleBookProxy(
            self.engine, rb
        )
    ...
    def patch(self, patch):
        ...
        """
        self.engine.handle("update_nodes", char=self.character.name, patch=patch)
...
class ThingMapProxy(CachingProxy):
    ...
    def _set_rulebook_proxy(self, rb):
        self.engine._character_rulebooks_cache[self.name]["thing"] = RuleBookProxy(
            self.engine, rb
        )
    ...
    def _cache_set_munge(self, k, v):
        return ThingProxy(
            self,
            *self.engine.handle("get_thing_special_stats", char=self.name, thing=k),
        )
    ...
    def _del_item(self, k):
        self.engine.handle(command="del_node", char=self.name, node=k, branching=True)
        del self._cache[k]
...
class PlaceMapProxy(CachingProxy):
    ...
    def _set_rulebook_proxy(self, rb):
        self.engine._character_rulebooks_cache[self.name]["place"] = RuleBookProxy(
            self.engine, rb
        )
...
class SuccessorsProxy(CachingProxy):
    ...
    def _cache(self):
        ...
        if self._orig not in succc:
            raise KeyError("No successors to this portal", self._charname, self._orig)
        return succc[self._orig]

    def _set_rulebook_proxy(self, k):
        raise NotImplementedError("Set the rulebook on the .portal attribute, not this")
    ...
    def _apply_delta(self, delta):
        raise NotImplementedError("Apply the delta on CharSuccessorsMappingProxy")
...
class CharSuccessorsMappingProxy(CachingProxy):
    ...
    def _set_rulebook_proxy(self, rb):
        self.engine._character_rulebooks_cache[self.name]["portal"] = RuleBookProxy(
            self.engine, rb
        )
    ...
    @property
    def _cache(self):
        return self.engine._character_portals_cache.successors.setdefault(self.name, {})
...
class PredecessorsProxy(MutableMapping):
    ...
    def __iter__(self):
        return iter(
            self.engine._character_portals_cache.predecessors[self._charname][self.name]
        )
    ...
    def __contains__(self, k):
        return (
            k
            in self.engine._character_portals_cache.predecessors[self._charname][
                self.name
            ]
        )

    def __getitem__(self, k):
        return self.engine._character_portals_cache.predecessors[self._charname][
            self.name
        ][k]
    ...
    def __setitem__(self, k, v):
        ...
        self.engine.handle("set_portal", (self._charname, k, self.name), branching=True)
...
class CharPredecessorsMappingProxy(MutableMapping, Signal):
    ...
    def __setitem__(self, k, v):
        ...
        for pred, proxy in v.items():
            self.engine._character_portals_cache.store(self.name, pred, k, proxy)
        self.engine.handle(
...
class CharStatProxy(CachingEntityProxy):
    ...
    def _set_rulebook_proxy(self, k):
        raise NotImplementedError("Set rulebooks on the Character proxy, not this")
...
class UnitMapProxy(Mapping):
    ...
    def _get_rulebook_proxy(self):
        return self.engine._character_rulebooks_cache[self.character.name]["unit"]
    ...
    def __iter__(self):
        yield from self.character.engine._character_units_cache[self.character.name]

    def __len__(self):
        return len(self.character.engine._character_units_cache[self.character.name])

    def __contains__(self, k):
        return k in self.character.engine._character_units_cache[self.character.name]

    def __getitem__(self, k):
        if k not in self:
            raise KeyError("{} has no unit in {}".format(self.character.name, k))
        return self.GraphUnitsProxy(self.character, self.character.engine.character[k])
    ...
    class GraphUnitsProxy(Mapping):
        ...
        def __contains__(self, k):
            cache = self.character.engine._character_units_cache[self.character.name]
...
class CharacterProxy(AbstractCharacter):
        ...
        for orig in g.adj:
            for dest, edge in g.adj[orig].items():
                if orig in self.portal and dest in self.portal[orig]:
                    ...
                    self.engine._portal_stat_cache[self.name][orig][dest] = edge
    ...
    def _set_rulebook_proxy(self, rb):
        self.engine._character_rulebooks_cache[self.name]["character"] = RuleBookProxy(
            self.engine, rb
        )
        ...
        for node, ex in delta.pop("nodes", {}).items():
            if ex:
                ...
                if node in self.place._cache:
                    ...
                    self.engine.warning(
                        "Diff deleted {} but it was never created here".format(node)
                    )
    ...
    def portals(self):
        yield from self.engine.handle(command="character_portals", char=self.name)
    ...
    def units(self):
        yield from self.engine.handle(command="character_units", char=self.name)
...
class CharacterMapProxy(MutableMapping, Signal):
    ...
    def __setitem__(self, k, v):
        ...
        self.engine.handle(command="set_character", char=k, data=v, branching=True)
...
class StringStoreProxy(Signal):
    ...
    def lang_items(self, lang=None):
        if lang is None or lang == self.language:
            ...
            yield from self.engine.handle(command="get_string_lang_items", lang=lang)
...
class FuncStoreProxy(Signal):
    ...
    def __delattr__(self, func_name):
        self.engine.handle(command="del_source", store=self._store, k=func_name)
...
class RandoProxy(Random):
    ...
    def seed(self, a=None, version=2):
        self._handle(cmd="call_randomizer", method="seed", a=a, version=version)

    def getstate(self):
        return self._handle(cmd="call_randomizer", method="getstate")

    def setstate(self, state):
        return self._handle(cmd="call_randomizer", method="setstate", state=state)
...
class EngineProxy(AbstractEngine):
    ...
    def switch_main_branch(self, branch: str) -> None:
        ...
        if self.branch != self.main_branch or self.turn != 0 or self._tick != 0:
            raise ValueError("Go to the start of time first")
        kf = self.handle("switch_main_branch", branch=branch, cb=self._set_time)
    ...
    def _replace_state_with_kf(self, result, **kwargs):
        ...
        for graph, stats in result["graph_val"].items():
            if "character_rulebook" in stats:
                chars[graph]._set_rulebook_proxy(stats.pop("character_rulebook"))
            if "unit_rulebook" in stats:
                chars[graph].unit._set_rulebook_proxy(stats.pop("unit_rulebook"))
            ...
            for key in list(stats):
                if key in chars[graph].stat and stats[key] == chars[graph].stat[key]:
                    del stats[key]
        ...
        for char, nodes in result["nodes"].items():
            for node, ex in nodes.items():
                if ex:
                    ...
                    if char in things and node in things[char]:
                        del things[char][node]
                        chars[char].thing.send(chars[char], key=node, value=None)
                        chars[char].node.send(chars[char], key=node, value=None)
                    if char in places and node in places[char]:
                        del places[char][node]
                        chars[char].place.send(chars[char], key=node, value=None)
                        chars[char].node.send(chars[char], key=node, value=None)
        for char, origs in result["edges"].items():
            for orig, dests in origs.items():
                for dest, exists in dests.items():
                    ...
                    chars[char].adj.send(chars[char], orig=orig, dest=dest, value=that)
                    chars[char].pred.send(chars[char], orig=orig, dest=dest, value=that)
    ...
    def _call_with_recv(self, *cbs, **kwargs):
        ...
        self.debug(
            "EngineProxy: received {}".format((cmd, branch, turn, tick, received))
        )
    ...
    def _upd_caches(self, command, branch, turn, tick, result):
        ...
        for rulebook, delta in rulebookdeltas.items():
            if rulebook not in self._rulebook_obj_cache:
                self._rulebook_obj_cache[rulebook] = RuleBookProxy(self, rulebook)
    ...
    def _add_character(self, char, data=None, **attr):
        ...
        for place, stats in placedata.items():
            if (
                ...
                self._character_places_cache[char][place] = PlaceProxy(character, place)
        ...
        for thing, stats in thingdata.items():
            ...
            if char not in self._things_cache or thing not in self._things_cache[char]:
                self._things_cache[char][thing] = ThingProxy(character, thing, loc)
...
class EngineProcessManager(object):
    ...
    def start(self, *args, **kwargs):
        ...
        if "logfile" in kwargs:
            ...
            del kwargs["logfile"]
        install_modules = (
            kwargs.pop("install_modules") if "install_modules" in kwargs else []
        )
        ...
        self._p.start()
        self._logthread = Thread(target=self.sync_log_forever, name="log", daemon=True)
