...
def test_keyframe_load(db):
    for graph in testgraphs:
        ...
        assert (graph.name,) in nodes_kf, "{} not in nodes cache".format(graph.name)
        assert (
        ...
        }, "{} not loaded correctly, got {}".format(graph.name, nodes_kf["trunk"][0][0])
        ...
        if graph.is_multigraph():
            for orig in graph.adj:
                for dest in graph.adj[orig]:
                    ...
                    assert this_edge["trunk"].rev_gettable(
                        0
                    ), "turn 0 not in trunk branch of edges cache for {}".format(
                        (graph.name, orig, dest)
                    )
                    assert this_edge["trunk"][0].rev_gettable(
                        0
                    ), "tick 0 not in turn 0 of trunk branch of edges cache for {}".format(
                        (graph.name, orig, dest)
                    )
                    assert db._edges_cache.keyframe[graph.name, orig, dest]["trunk"][0][
                        0
                    ] == {
            ...
            for orig in graph.adj:
                for dest in graph.adj[orig]:
                    ...
                    assert db._edges_cache.keyframe[graph.name, orig, dest]["trunk"][0][
                        0
                    ] == {0: True}, "{} not loaded".format((graph.name, orig, dest))
        ...
        for edge in graph.edges:
            if graph.is_multigraph():
                ...
                assert (
                    db._edge_val_cache.keyframe[(graph.name,) + edge + (0,)]["trunk"][
                        0
                    ][0]
                    == graph.edges[edge]
                )
...
def test_keyframe_unload(tmpdbfile):
    ...
    with ORM("sqlite:///" + tmpdbfile) as orm:
        ...
        if (
            ...
            (0, 0),
            (0, 1),
        ) in orm._edges_cache.keyframe and "trunk" in orm._edges_cache.keyframe[
            "g", (0, 0), (0, 1)
        ]:
            assert 0 not in orm._edges_cache.keyframe["g", (0, 0), (0, 1)]["trunk"]
        assert not orm._time_is_loaded("trunk", 1)
