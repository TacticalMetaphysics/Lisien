...
class ConnectionHolder:
    ...
    def __init__(self, dbstring, connect_args, inq, outq, fn, tables, gather=None):
        self.lock = Lock()
...
class QueryEngine(object):
    ...
    def __init__(self, dbstring, connect_args, pack=None, unpack=None, gather=None):
    ...
    def keyframe_graph_insert(self, graph, branch, turn, tick, nodes, edges, graph_val):
    ...
    def keyframes_graphs(self):
        unpack = self.unpack
        for graph, branch, turn, tick in self.call_one("keyframes_graphs_list"):
    ...
    def new_turn(self, branch, turn, end_tick=0, plan_end_tick=0):
        return self.call_one("turns_insert", branch, turn, end_tick, plan_end_tick)

    def update_turn(self, branch, turn, end_tick, plan_end_tick):
        return self.call_one("update_turns", end_tick, plan_end_tick, branch, turn)
    ...
    def graph_val_dump(self) -> Iterator[GraphValRowType]:
        ...
        unpack = self.unpack
        for graph, key, branch, turn, tick, value in self.call_one("graph_val_dump"):
            yield (
                unpack(graph),
                unpack(key),
                branch,
    ...
    def graphs_insert(self, graph, branch, turn, tick, typ):
        self.call_one("graphs_insert", self.pack(graph), branch, turn, tick, typ)
    ...
    def nodes_dump(self) -> Iterator[NodeRowType]:
        ...
        self._flush_nodes()
        unpack = self.unpack
        for graph, node, branch, turn, tick, extant in self.call_one("nodes_dump"):
            yield (
                unpack(graph),
                unpack(node),
                branch,
    ...
    def load_nodes(
        ...
        return list(
            self.iter_nodes(graph, branch, turn_from, tick_from, turn_to, tick_to)
        )
    ...
    def _put_window_tick_to_tick(self, branch, turn_from, tick_from, turn_to, tick_to):
        putkwargs = {
            "branch": branch,
            "turn_from": turn_from,
            "tick_from": tick_from,
    ...
    def _get_one_window(self, ret, branch, turn_from, tick_from, turn_to, tick_to):
        ...
        while isinstance(got := self._outq.get(), list):
            for graph, key, turn, tick, val in got:
                ...
                ret[graph]["graph_val"].append((graph, key, branch, turn, tick, val))
        assert got == (
            "end",
            "graph_val",
            branch,
    ...
    def load_node_val(
        ...
        return list(
            self.iter_node_val(graph, branch, turn_from, tick_from, turn_to, tick_to)
        )
    ...
    def load_edges(
        ...
        return list(
            self.iter_edges(graph, branch, turn_from, tick_from, turn_to, tick_to)
        )
    ...
    def _flush_edges(self):
        ...
        if not self._edges2set:
            return
        self.call_many("edges_insert", map(self._pack_edge2set, self._edges2set))
        self._edges2set = []
    ...
    def load_edge_val(
        ...
        return list(
            self.iter_edge_val(graph, branch, turn_from, tick_from, turn_to, tick_to)
        )
