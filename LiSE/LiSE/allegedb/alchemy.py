...
def tables_for_meta(meta):
    ...
    Table(
        ...
        CheckConstraint(
            "type IN " "('Graph', 'DiGraph', 'MultiGraph', 'MultiDiGraph', 'Deleted')"
        ),
        sqlite_with_rowid=False,
    )
    kfs = Table(
...
def queries_for_table_dict(table):
    ...
    r = {
        ...
        .delete()
        .where(table["global"].c.key == bindparam("key")),
        "graphs_types": select(table["graphs"].c.graph, table["graphs"].c.type),
...
def compile_sql(dialect, meta):
    return {k: v.compile(dialect=dialect) for (k, v) in gather_sql(meta).items()}
