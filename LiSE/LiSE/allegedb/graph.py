...
class AbstractEntityMapping(AllegedMapping, ABC):
    ...
    def __setitem__(self, key, value):
        ...
        if value is None:
            raise ValueError("allegedb uses None to indicate that a key's been deleted")
        branch, turn, tick = self.db._nbtt()
        try:
...
class GraphNodeMapping(AllegedMapping):
    ...
    def __iter__(self):
        ...
        return self.db._nodes_cache.iter_entities(self.graph.name, *self.db._btt())
    ...
    def __len__(self):
        ...
        return self.db._nodes_cache.count_entities(self.graph.name, *self.db._btt())
    ...
    def __delitem__(self, node):
        ...
        branch, turn, tick = self.db._nbtt()
        self.db.query.exist_node(self.graph.name, node, branch, turn, tick, False)
        self.db._nodes_cache.store(self.graph.name, node, branch, turn, tick, False)
...
class GraphSuccessorsMapping(GraphEdgeMapping):
    ...
    def __contains__(self, key):
        return (
            self.db._edges_cache.count_successors(self.graph.name, key, *self.db._btt())
            > 0
        )
    ...
    def __repr__(self):
        ...
        return "<{}.{} object containing {}>".format(
            cls.__module__,
            cls.__name__,
            {k: {k2: dict(v2) for (k2, v2) in v.items()} for (k, v) in self.items()},
        )
...
class DiGraph(networkx.DiGraph):
    ...
    def _nodes_state(self):
        return {
            noden: {k: v for (k, v) in unwrapped_dict(node).items() if k != "name"}
    ...
    def _val_state(self):
        return {k: v for (k, v) in unwrapped_dict(self.graph).items() if k != "name"}
...
class GraphsMapping(MutableMapping):
    ...
    def __contains__(self, name):
        ...
        try:
            return self.orm._graph_cache.retrieve(name, *self.orm._btt()) != "Deleted"
        except KeyError:
            return False
