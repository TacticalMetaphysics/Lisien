...
class StructuredDefaultDict(dict):
    ...
    def __getitem__(self, k):
        ...
        if layer == 1:
            ...
            ret = StructuredDefaultDict(layer - 1, typ, args_munger, kwargs_munger)
            ret.parent = self
            ret.key = k
...
class Cache:
                ...
                if keycache2 and keycache2.rev_gettable(turn):
                    ...
                    if tick not in keycache3:
                        if keycache3.rev_gettable(tick):
                            ...
                            ret = keycache3[tick] = keys_before.union(added).difference(
                                deleted
                            )
                            # assert ret == get_adds_dels(
                            # keys[parentity], branch, turn, tick)[0]  # slow
    ...
    def remove_character(self, character):
        ...
        with lock:
            ...
            for branch, turn, tick, parent, entity, key in todel:
                self._remove_btt_parentikey(branch, turn, tick, parent, entity, key)
    ...
    def _base_retrieve(self, args, store_hint=True, retrieve_hint=True, search=False):
        """Hot code.
                ...
                if brancs is not None and brancs.rev_gettable(r):
                    if r in brancs and brancs[r].rev_gettable(t):
                        ...
                        if b in keyframes and r in keyframes[b]:
                            ...
                            if search:
                                brancs.search(r)
                            if kfbr.rev_before(t, search=search) is not None and (
                        ...
                        if b in keyframes and keyframes[b].rev_gettable(r - 1):
                            kfb = keyframes[b]
                            if kfb.rev_before(r - 1, search=search) is not None and (
...
class EdgesCache(Cache):
        ...
        *,
        ...
        for (grap, org, dest), kfg in its:  # too much iteration!
            if (grap, org) != (graph, orig):
                continue
            for branc, trn, tck in itparbtt(branch, turn, tick, stoptime=stoptime):
                if branc not in kfg:
                    continue
                kfgb = kfg[branc]
                if trn in kfgb:
    ...
    def iter_predecessors(
        ...
        *,
        ...
        if self.db._no_kc:
            yield from self._adds_dels_predecessors((graph, dest), branch, turn, tick)[
                0
            ]
            return
        if forward is None:
    ...
    def count_successors(
        ...
        *,
        ...
        if self.db._no_kc:
            return len(self._adds_dels_successors((graph, orig), branch, turn, tick)[0])
        if forward is None:
            forward = self.db._forward
        return len(
            self._get_destcache(graph, orig, branch, turn, tick, forward=forward)
        )
    ...
    def count_predecessors(
        ...
        *,
        ...
        if self.db._no_kc:
            return len(
                self._adds_dels_predecessors((graph, dest), branch, turn, tick)[0]
            )
        if forward is None:
            forward = self.db._forward
        return len(
            self._get_origcache(graph, dest, branch, turn, tick, forward=forward)
        )
