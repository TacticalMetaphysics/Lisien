...
from .window import HistoricKeyError, WindowDict, update_backward_window, update_window
...
class TimeSignalDescriptor:
    ...
    def __set__(self, inst: "ORM", val: Tuple[str, int]):
        ...
        if branch_now in branches:
            tick_now = e._turn_end_plan.setdefault((branch_now, turn_now), tick_then)
            ...
            if not e._planning and (
                turn_now > turn_end or (turn_now == turn_end and tick_now > tick_end)
            ):
                branches[branch_now] = (
                    parent,
                    turn_start,
...
class ORM:
    ...
    def _get_edge(self, graph, orig, dest, idx=0):
        ...
        if not edge_exists(graphn, orig, dest, idx):
            raise KeyError(
                "No such edge: {}->{}[{}] in {}".format(orig, dest, idx, graphn)
            )
    ...
    def _arrange_caches_at_time(self, _, *, branch: str, turn: int, tick: int) -> None:
    ...
    def _get_branch_delta(
        ...
        def setgraphval(delta: DeltaDict, graph: Key, key: Key, val: Any) -> None:
        ...
        def setnodeval(
            ...
            if (graphstat := delta.setdefault(graph, {})) is not None:
                if (
                    ...
                    return
                graphstat.setdefault("node_val", {}).setdefault(node, {})[key] = value
        ...
        def setedgeval(
            ...
            if (graphstat := delta.setdefault(graph, {})) is not None:
                ...
                if (
                    ...
                    return
                graphstat.setdefault("edge_val", {}).setdefault(orig, {}).setdefault(
                    dest, {}
                )[key] = value
        ...
        if branch in ebranches:
            updater(
                partial(setedge, delta, lambda g: graph_objs[g].is_multigraph()),
                ebranches[branch],
            )

        if branch in evbranches:
            updater(
                partial(setedgeval, delta, lambda g: graph_objs[g].is_multigraph()),
                evbranches[branch],
            )
            ...
            for graph, orig, dest, idx, exists in ebranches[branch][turn][
                ...
                if graph_objs[graph].is_multigraph():
                    if graph in delta and (
                        ...
                        continue
                    delta.setdefault(graph, {}).setdefault("edges", {}).setdefault(
                        orig, {}
                    )[dest] = bool(exists)
                else:
                    if graph in delta and (
    ...
    def _init_caches(self):
        ...
        self._branches: Dict[str, Tuple[Optional[str], int, int, int, int]] = {}
        ...
        self._turn_end_plan: Dict[Tuple[str, int], int] = defaultdict(lambda: 0)
    ...
    def _has_graph(self, graph, branch=None, turn=None, tick=None):
        ...
        if tick is None:
            tick = self.tick
        try:
            return self._graph_cache.retrieve(graph, branch, turn, tick) != "Deleted"
        except KeyError:
            return False
    ...
    def _get_kf(self, branch: str, turn: int, tick: int, copy=True) -> Dict[
        ...
        for graph, orig, dest, idx in self._edge_val_cache.keyframe:
            assert idx == 0, "Not doing idx other than 0 until multigraphs come back"
            try:
    ...
    def _load_plans(self) -> None:
        ...
        for plan, turn, tick in self.query.plan_ticks_dump():
            ...
            turn_end_plan[branch, turn] = max((turn_end_plan[branch, turn], tick))
    ...
    def _snap_keyframe_de_novo_graph(
        ...
        try:
            graphs_keyframe = self._graph_cache.get_keyframe(branch, turn, tick)
        except KeyframeError:
            graphs_keyframe = {
                g: "DiGraph" for g in self._graph_cache.iter_keys(branch, turn, tick)
            }
        ...
        for orig, dests in edges.items():
            for dest, vals in dests.items():
                ec.set_keyframe((graph, orig, dest), branch, turn, tick, {0: True})
                evc.set_keyframe((graph, orig, dest, 0), branch, turn, tick, vals)
        self._graph_val_cache.set_keyframe((graph,), branch, turn, tick, graph_val)
    ...
    def _copy_kf(self, branch_from, branch_to, turn, tick):
        ...
        """
        try:
            graph_keyframe = self._graph_cache.get_keyframe(branch_from, turn, tick)
        except KeyframeError:
            graph_keyframe = {
                graph: self._graph_cache.retrieve(graph, branch_from, turn, tick)
                for graph in self._graph_cache.iter_entities(branch_from, turn, tick)
            }
        ...
        for graph in (
            ...
            if deltg:
                if graph in graph_val_keyframe:
                    if "units" in graph_val_keyframe[graph] and "units" in deltg:
    ...
    def _build_keyframe_window(
        ...
        if latest_past_keyframe and self._loaded[branch][2:] < latest_past_keyframe[1:]:
    ...
    def _load_between(
        ...
        graphvalrows = []
        graphsrows = list(
            self.query.graphs_types(branch, turn_from, tick_from, turn_to, tick_to)
        )
    ...
    @world_locked
    def _read_at(self, branch: str, turn: int, tick: int) -> Tuple[
        ...
        (latest_past_keyframe, earliest_future_keyframe) = self._build_keyframe_window(
            branch_now,
            turn_now,
            tick_now,
            loading=True,
        ...
        if latest_past_keyframe is None:
            if earliest_future_keyframe is None:
                return (
                    ...
                    None,
                    list(
                        self.query.graphs_types(self.query.globl["main_branch"], 0, 0)
                    ),
    ...
    def unload(self) -> None:
        ...
        for past_branch, past_turn, past_tick in iter_parent_btt(branch, turn, tick):
                ...
                for kfturn, kfticks in kfd[past_branch].items():
                    ...
                    for kftick in kfticks:
                        if (
                            ...
                            if (
                                kfturn < turn or (kfturn == turn and kftick < tick)
                            ) and (
                                kfturn > early_turn
                                or (kfturn == early_turn and kftick > early_tick)
                            ):
        ...
        for past_branch, (
            ...
            if early_turn == late_turn:
                ...
                if past_branch in self._keyframes_dict:
                    for turn, ticks in self._keyframes_dict[past_branch].items():
                        if early_turn == turn:
                            for tick in ticks:
                                if early_tick <= tick:
    ...
    def _time_is_loaded(self, branch: str, turn: int = None, tick: int = None) -> bool:
        loaded = self._loaded
        if branch not in loaded:
            return False
        if turn is None:
        ...
        if tick is None:
            ...
            early_turn, early_tick, late_turn, late_tick = loaded[branch]
            return (early_turn, early_tick) <= (turn, tick) <= (late_turn, late_tick)
    ...
    def is_ancestor_of(self, parent: str, child: str) -> bool:
        ...
        if child not in self._branches:
            raise ValueError(
                "The branch {} seems not to have ever been created".format(child)
            )
    ...
    def _set_branch(self, v: str):
        ...
        if (curturn > end_turn or (curturn == end_turn and tick > end_tick)) or (
            curturn < start_turn or (curturn == start_turn and tick < start_tick)
        ):
            self._load_at(v, curturn, tick)

    @world_locked
    def _copy_plans(self, branch_from: str, turn_from: int, tick_from: int) -> None:
        ...
        for plan_id in self._branches_plans[branch_from]:
            ...
            for turn, ticks in list(plan_ticks[plan_id].items()):
                ...
                for tick in ticks:
                    ...
                    plan_ticks_uncommitted.append((self._last_plan, turn, tick))
    ...
    def _set_turn(self, v: int):
        ...
        if v < turn_start:
            ...
            raise OutOfTimelineError(
                f"The turn number {v} occurs after the end " f"of the branch {branch}",
                self.branch,
                self.turn,
                self.tick,
                self.branch,
    ...
    def _nbtt(self) -> Tuple[str, int, int]:
        ...
        if turn_end[branch_turn] > tick:
            raise HistoricKeyError(
                ...
                "Go to tick {} to change things".format(turn, turn_end[branch_turn])
            )
        ...
        if self._planning:
            ...
            if (turn, tick) in plan_ticks[last_plan]:
                raise OutOfTimelineError(
                    ...
                    "but that time already happened".format((branch, turn, tick)),
                    self.branch,
                    self.turn,
                    self.tick,
                    self.branch,
    ...
    def _nudge_loaded(self, branch: str, turn: int, tick: int) -> None:
        ...
        if branch in loaded:
            ...
            elif turn > future_turn or (turn == future_turn and tick > future_tick):
    ...
    def new_digraph(self, name: Key, data: dict = None, **attr) -> DiGraph:
        ...
        if data and isinstance(data, nx.Graph):
            ...
            self._init_graph(name, "DiGraph", [data._node, data._succ, data.graph])
        else:
        ...
        if stoptime:
            ...
            while branch in _branches and not stopping:
                ...
                if branch == stopbranch:
                    stopping = True
                    if trn < stopturn or (
                        trn == stopturn and (stoptick is None or tck <= stoptick)
                    ):
                        return
                yield branch, trn, tck
        else:
    ...
    def _edge_exists(self, character: Key, orig: Key, dest: Key, idx=0) -> bool:
