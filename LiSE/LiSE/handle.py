...
class EngineHandle:
    ...
    def _pack_delta(self, delta) -> Tuple[SlightlyPackedDeltaType, bytes]:
        ...
        for char, chardelta in delta.items():
            if chardelta is None or chardelta == {"name": None}:
                pchar = pack(char)
                slightly_packed_delta[pchar] = mostly_packed_delta[pchar] = None
                continue
            ...
            if "nodes" in chardelta:
                nd = chard[NODES] = {
                    pack(node): pack(ex) for node, ex in chardelta.pop("nodes").items()
                }
    ...
    def update_nodes(self, char: Key, patch: Dict):
        ...
        with self._real.batch(), timer("EngineHandle.update_nodes", self.debug):
    ...
    def add_thing(self, char: Key, thing: Key, loc: Key, statdict: Dict) -> None:
    ...
    def thing_go_to_place(self, char: Key, thing: Key, place: Key, weight: Key) -> int:
    ...
    def set_portal_stat(self, char: Key, orig: Key, dest: Key, k: Key, v) -> None:
    ...
    def call_stored_function(
        ...
        delta = self._real._get_branch_delta(branch, turn, tick, turn_now, tick_now)
        return res, delta
    ...
    def rules_handled_turn(
        ...
        return {
            "character": eng._character_rules_handled_cache.handled_deep[branch][turn],
            ...
            "portal": eng._portal_rules_handled_cache.handled_deep[branch][turn],
        }
