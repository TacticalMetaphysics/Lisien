...
@pytest.fixture(
    scope="function",
    params=[
        lambda eng: kobold.inittest(eng, shrubberies=20, kobold_sprint_chance=0.9),
        # college.install,
        # sickle.install
    ],
)
...
@pytest.fixture(scope="function", params=["parallel-execution", "serial-execution"])
def engy(tmp_path, request):
    if request.config.getoption("serial") and request.param == "parallel-execution":
