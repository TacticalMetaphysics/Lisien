...
class StringStore(MutableMapping, Signal):
    ...
    def _load_language(self, lang):
        if hasattr(self, "_cache"):
            with open(os.path.join(self._prefix, self.language + ".json"), "w") as outf:
    ...
    def save(self, reimport=False):
        ...
        with open(os.path.join(self._prefix, self._language + ".json"), "w") as outf:
...
class FunctionStore(Signal):
    ...
    def __init__(self, filename):
        if not filename.endswith(".py"):
            raise ValueError("FunctionStore can only work with pure Python source code")
        super().__init__()
