...
for minor in $(seq 12 14); do
    ...
    /usr/local/bin/python3.$minor -m pip install --root-user-action ignore Cython==0.29.34
    USE_SDL3=0 USE_X11=1 /usr/local/bin/python3.$minor -m pip wheel --no-deps .
    /usr/local/bin/python3.$minor -m pip install --root-user-action ignore kivy*-cp3$minor-linux_x86_64.whl
done
cd ..
mkdir /wheels
mv kivy-$COMMIT_HASH/*.whl /wheels/
...
RUN python -m pip install --root-user-action ignore pytest tox
# install deployment dependencies
RUN /usr/local/bin/python3.14 -m pip install --root-user-action ignore \
    twine \
    sphinx \
    isort \
    ruff \
